...
// 헬퍼 프로세스엔 창이 없어 영영 그리로 못 간다. 그 표는 소켓 앞에 선다.
 ...
 *  by="shell" 은 셸이 스스로 답한 줄이다. 순서는 남기되 헬퍼 요구 목록에는 섞이지 않는다 —
 *  창·웹뷰·네이티브 표면은 헬퍼로 갈 수 없으므로(다른 프로세스엔 창이 없다) 그 목록에 실리면
...
// ── 셸의 것 — 네이티브 명령 ──────────────────────────────────────────────────
// 창·웹뷰·네이티브 표면 명령은 헬퍼로 갈 수 없다: 다른 프로세스엔 창이 없다. 그래서 이 표가
