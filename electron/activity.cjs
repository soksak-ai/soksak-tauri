...
// 없어 적재까지만 하고 도장 찍힌 항목을 응답에 실어 준다. 창은 셸의 것이므로 부채질은 셸이
...
// 더하지도 않는다).
//
// electron 을 require 하지 않는다 — 셸을 띄워야만 검증되는 코드는 사실상 검증되지 않는다.

/** 이 명령의 답만 부채질 대상이다. 코어의 발행 진입점과 같은 이름. */
const ACTIVITY_PUBLISH = "activity_publish";

/** 창이 받는 사건 이름. 코어의 broadcast 와 같아야 프론트가 셸을 가리지 않는다. */
...
const NOT_AN_ENTRY = "SHELL_ACTIVITY_NOT_AN_ENTRY";
 ...
 * 목록을 걷는다. 이 셸에서는 그 둘이 같은 집합이다 — 자식 표면을 만들지 않으므로 창
 * 레지스트리가 곧 받는 쪽 전부다(webview_list 가 그 사실을 값으로 답한다). 셸이 자식
