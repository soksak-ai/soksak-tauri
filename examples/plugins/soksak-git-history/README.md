...
```bash
# GitHub 단축형
sok plugin.install '{"source":"<user>/soksak-git-history"}'

# 로컬 경로(이 예제 디렉토리)
sok plugin.install '{"source":"/path/to/repo/examples/plugins/soksak-git-history"}'

# 활성화(동의는 앱 UI 에서 사람이 직접)
sok plugin.enable '{"id":"soksak-git-history"}'
```
