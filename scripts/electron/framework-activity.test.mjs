...
// 활동 부채질 — cored가 적재해 돌려준 항목을 프레임워크가 자기 창에 뿌리는가.
//
// 발행은 세 단이다: 적재(도장·단조) · 부채질(창 emit) · 영속(records 쓰기). cored 프로세스엔
// 창이 없어 적재까지만 하고 도장 찍힌 항목을 답에 실어 준다. 부채질을 프레임워크가 안 하면 프론트는
...
// 그대로가 손에 잡힌다(framework-invoke.test.mjs 와 같은 방식). 여기서는 whenReady 를 풀어 창까지
// 만든다 — 부채질의 대상 목록이 곧 프레임워크의 창 레지스트리이기 때문이다.
...
/** 창 대역 — 프레임워크가 미는 사건을 채널째 기록한다(렌더러가 preload 에서 받는 그 모양). */
function fakeWindow() {
  const sent = [];
  return {
    sent,
...
/**
 * 프레임워크를 적재한다. whenReady 를 풀어 프레임워크가 자기 창을 만들게 한다 — 부채질 대상은 그 레지스트리다.
 * 돌려주는 created 는 프레임워크가 만든 창들(만든 순서).
 */
async function loadFramework(socketPath) {
...
/**
 * 프리로드를 적재한다 — 렌더러가 프레임워크를 만나는 유일한 창구. 프레임워크가 보낸 봉투를 여기가 풀지
...
function loadPreload() {
  ...
  // 프레임워크는 창을 만들 때 라벨을 주입한다 — 프리로드는 그것 없이는 적재되지 않는다(폴백 없음).
...
describe("적재분은 창에 사건으로 도착한다", () => {
  it("cored가 적재해 돌려준 항목을 프레임워크가 자기 창에 뿌린다", async () => {
    ...
    const { handlers, created } = await loadFramework(mock.socketPath);
  ...
  it("적재분이 아닌 답은 이름을 달고 거절한다 — 아닌 것을 창에 밀지 않는다", async () => {
    ...
    expect(r.code).toBe("FRAMEWORK_ACTIVITY_NOT_AN_ENTRY");
    expect(r.command).toBe("activity_publish");
    expect(win.sent).toEqual([]);
  });
});

describe("봉투 — 프레임워크가 보낸 것을 프리로드가 그대로 푼다", () => {
  it("구독자가 받는 것은 항목 그 자체다 — 코어의 listen(\"activity\") 와 같은 모양", async () => {
    // 프레임워크가 창에 넣는 봉투와 프리로드가 푸는 봉투가 어긋나면 사건은 창까지 가고도 구독자에게
    // 닿지 않는다. 두 쪽을 실물로 이어 본다 — 프레임워크도 렌더러도 띄우지 않고.
    ...
    // 어댑터(src/framework/electron.ts)는 이 값을 { payload } 로 감싸 앱에 준다 — 그래서 앱이
...
// 창이 여럿일 때의 규칙 — 프레임워크는 whenReady 에서 창을 하나만 만들므로 배선 검증으로는 표현되지
