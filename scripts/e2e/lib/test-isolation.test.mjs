...
// 실행에서만 실패했고, 원인은 두 스위트가 /tmp 아래 같은 이름을 쓴 것이었다.
//
...
describe("테스트는 고정 경로를 소유하지 않는다", () => {
  it("os.tmpdir 아래 고정 이름을 쓰는 테스트가 없다", () => {
    ...
    for (const file of globSync("scripts/e2e/**/*.test.mjs", { cwd: ROOT })) {
      ...
      for (const [index, text] of source.split("\n").entries()) {
        const line = index;
        // 리터럴 "/tmp/..." 는 이 프로세스만의 자리가 아니다. mkdtemp 나 pid 로 자기 자리를 받아라.
        //
        ...
        if (/["'`]\/tmp\//.test(text) && !text.trimStart().startsWith("//") && !plantsViolation) {
  ...
  it("고정 경로를 심으면 잡는다", () => {
    const planted = 'const root = "/tmp/soksak-fixed-name";';
    expect(/["\'`]\/tmp\//.test(planted)).toBe(true);
    // 픽스처를 짓는 줄(write)은 규칙 대상이 아니다 — 그 차이도 함께 고정한다.
    expect(/\bwrite\(/.test('write("bad.mjs", \'"/tmp/x.sock"\');')).toBe(true);
  });
});
