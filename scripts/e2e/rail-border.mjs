...
//
// ── 알려진 미해결(2026-07-27) ────────────────────────────────────────────────
// 배율 1 창을 `window.resize` 로 **최초 높이보다 크게** 만들면, 레일 열에서 그 선 아래가
// 옛 픽셀 그대로 남는다. 카드 하단 테두리가 그 띠에 들어가면 사라진 것처럼 보인다.
// 확정된 사실:
//   · 선은 실제로 그려진다 — 무관한 재도색(탭 전환) 뒤 같은 자리에서 x 전 구간 값 64 로 관측.
//     즉 "안 그려짐"이 아니라 "새로 드러난 띠가 다시 칠해지지 않음"이다.
//   · 레이아웃·계산값 모두 정상: 레일·카드 rect 가 정확히 성장하고 computed
//     border-bottom-width 는 1px, 색도 토큰 값 그대로다.
//   · 2px 로 두면 사라지는 높이가 정확히 1px 밀린다(607→608) — 고정된 선 아래가 잘리는
//     것이고, 그 선은 창의 최초 높이다. 2.5초를 더 기다려도 복구되지 않는다.
//   · 죽은 자리는 레일 열뿐이다. 같은 행에서 콘텐츠 칸의 테두리는 정상 도색된다.
// 배제됨(실측):
//   · 카드 라운드 클립 마스크 — 라운드를 빼도 임계 동일.
//   · 합성 레이어 승격 — 레일·카드 모두 willChange auto / transform none / filter none /
//     contain none / opacity 1. will-change 토글 보정도 무효.
//   · 네이티브 표면 — webview.surfaces 의 엔진 표면 frame 은 콘텐츠 영역(x≥307)뿐.
//   · 점진(1px×N) vs 일괄(+11) 차이 없음. 투영 종류(북마크/파일트리) 차이 없음
//     (그렇게 보였던 것은 탭 전환이 곧 재도색이라 생긴 착시였다).
// 남은 사실: 레일은 **상시** clip-path 를 단다(사이드바가 브라우저 네이티브 표면 위를
//   칠하지 않는다는 규정 — lib/railHoleClip). 클립을 없애는 선택지는 없다. 클립이 커질 때
//   드러난 영역을 프레임 안에서 해제·재적용으로 무효화해 봤으나 효과 없었다(원복함).
