...
export const LOGIC_LEDGER = new Map([
  ...
  [
    "project_claim",
    [5, "점유 지도 판정 + 변이 통지. **옮기지 않는다** — 지도의 수명이 곧 창의 수명이라 창을 소유한 쪽이 져야 한다(cored 가 쥐면 재기동 뒤에도 죽은 창의 점유가 남아 그 프로젝트를 다시 못 연다. cored 의 UNSERVED 가 같은 사유를 적었다). 그래서 구현은 둘이고, 규칙이 갈리지 않는다는 것은 같은 픽스처가 묶는다(fixtures/project-claims.json — Rust ProjectRegistry 검사와 JS 검사가 그 파일 하나를 읽는다). 2026-08-01 4→5: 지도가 **바뀌었을 때만** 알리는 한 줄. 이 프레임워크엔 발행이 아예 없어서 프로젝트를 점유·해제해도 같은 프로세스의 다른 창이 몰랐다 — 창에 뿌리는 일은 프레임워크만 할 수 있다"],
  ],
  [
    "project_release",
    [2, "같은 지도의 소유자 판정 + 변이 통지 — 같은 픽스처가 함께 묶는다(해제는 소유 창만). 통지 한 줄은 소유자 판정 갈래 안에 들어가 수가 그대로다"],
  ],
]);
