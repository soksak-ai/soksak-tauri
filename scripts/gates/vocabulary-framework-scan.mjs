...
export const REAL_SHELL_PATHS = [
  ["src-tauri/src/pty.rs", "PTY 세션의 임자 — 셸을 스폰하고 env·통합·생존을 소유한다"],
  ["src-tauri/src/daemon.rs", "Procfile 데몬이 로그인 셸을 인자로 받아 스폰한다"],
  ["src-tauri/src/login_shell.rs", "`$SHELL` 을 읽는 저장소의 유일한 자리"],
  ["src-tauri/src/ambient_gate.rs", "앰비언트 표에 `SHELL`·`ZDOTDIR` 이 항목으로 산다"],
  ["src-tauri/scripts/shell-integration.zsh", "셸 통합 스크립트 본체(OSC 133/633)"],
...
export const IDENTIFIER_RESIDUE = new Map([
  ["a_sink_needs_no_shell_type", "src-tauri/src/{stream_sink,activity_sink}.rs — 프레임워크 타입 없이 서는 출구"],
  ["the_sink_seam_needs_no_shell_type", "src-tauri/src/watcher.rs — 같은 축"],
  ["an_oracle_needs_no_shell_type", "src-tauri/src/window_oracle.rs — 같은 축"],
  ["a_dispatch_needs_no_shell_type", "src-tauri/src/command_dispatch.rs — 같은 축"],
  ["a_fire_needs_no_shell_type", "src-tauri/src/schedule.rs — 같은 축"],
  ["a_state_built_from_an_identity_needs_no_shell", "src-tauri/src/secrets.rs — 같은 축"],
  ["admitting_needs_no_shell_and_does_not_fan_out", "src-tauri/src/activity.rs — 같은 축"],
  ["application_shell_denies_external_and_privileged_navigation", "src-tauri/src/navigation_policy.rs — 앱 웹뷰(프레임워크)의 항행 정책"],
  ["application_shell_allows_only_the_declared_dev_origin", "src-tauri/src/navigation_policy.rs — 같은 축"],
  ["shellhost", "src-tauri/src/{identity,activity_sink}.rs 주석 — 이 이름의 trait 은 없다(죽은 참조)"],
...
export const PROSE_LEDGER = new Map([
  ["src-tauri/src/ws.rs", [4, "[프레임워크] \"셸 타입\" — ws 읽기 루프가 프레임워크 타입을 모른다는 서술"]],
  ["src-tauri/src/network.rs", [2, "[프레임워크] \"셸 진입점\"·\"셸 쪽\" — 프레임워크 위임 래퍼"]],
  ["src-tauri/src/process.rs", [3, "[프레임워크] \"셸 타입(State·Window·Channel)\""]],
  ["src-tauri/src/runtime_dep.rs", [1, "[프레임워크] \"셸 진입점(무논리 위임)\""]],
  ["src-tauri/src/clipboard.rs", [3, "[프레임워크] \"셸 없이\"·\"셸 타입\""]],
  ["src-tauri/src/stream_sink.rs", [1, "[프레임워크] \"셸을 어댑터 뒤로\""]],
  ["src-tauri/src/window_oracle.rs", [1, "[프레임워크] \"셸과 스트림 출구\""]],
  ["src-tauri/src/watcher.rs", [1, "[프레임워크] \"셸 타입 없이\""]],
  ["src-tauri/src/activity.rs", [4, "[프레임워크] \"셸 타입\"·\"셸에 넘긴다\""]],
  ["src-tauri/src/activity_sink.rs", [1, "[프레임워크] \"셸(ShellHost)\" — 경계 열거"]],
  ["src-tauri/src/identity.rs", [2, "[프레임워크] \"셸(ShellHost)\"·\"값의 출처가 셸\""]],
  ["src-tauri/src/service.rs", [1, "[프레임워크] \"셸 어댑터\"·\"두 번째 셸\""]],
  ["src-tauri/src/service_tests.rs", [3, "[프레임워크] \"셸 타입 0\"·\"셸 어댑터\"·\"두 번째 셸\""]],
  ["src-tauri/src/webview_health.rs", [1, "[프레임워크] \"런타임 셸(브레이커 소유 + 배선)\""]],
  ["src-tauri/src/ai_session.rs", [1, "[프레임워크] \"여기 남은 것은 셸 …\""]],
  ["src-tauri/src/navigation_policy.rs", [2, "[UI] \"app shell\" — 앱 웹뷰의 항행 정책"]],
  ["src-tauri/crates/soksak-core/src/lib.rs", [2, "[프레임워크] \"셸이 들어 있지 않은\"·\"셸에 안 묶임\""]],
  ["src-tauri/crates/soksak-core/src/kv.rs", [3, "[프레임워크] \"자원이지 셸이 아니다\"·\"무-셸 게이트\"·\"셸 타입\""]],
  ["src-tauri/crates/soksak-core/src/identity.rs", [2, "[프레임워크] \"앱은 셸 설정에서\"·\"붙는 것은 셸의 일\""]],
  ["src-tauri/crates/soksak-core/src/pathx.rs", [2, "[진짜셸] 틸드 확장은 명령 해석기의 일 — \"우리는 셸이 아니다\""]],
  ["src/lib/safeListen.ts", [2, "[프레임워크] \"셸 경계 뒤\"·\"셸 내부 맵\""]],
  ["src/lib/windowEvents.ts", [2, "[프레임워크] \"전역 listen(셸 경계)\"·\"셸 런타임 밖\""]],
  ["src/lib/notify.test.ts", [1, "[프레임워크] \"셸은 경계 하나로 mock\" — 죽은 참조 ../platform 동반"]],
  ["src/lib/imeKeys.ts", [2, "[프레임워크] \"셸을 갈아끼우면\"·\"어느 셸에서\""]],
  ["src/lib/imeLedger.test.ts", [1, "[프레임워크] \"셸 교체 후\""]],
  ["src/plugins/deps.ts", [1, "[프레임워크] \"전역 listen(셸 경계의 브로드캐스트)\""]],
  ["src/commands/catalogDom.test.ts", [1, "[프레임워크] \"셸은 경계 하나로 mock\""]],
  ["src/state/theme.ts", [1, "[프레임워크] \"셸 런타임 없음(jsdom)\""]],
  ...
  ["scripts/e2e/lib/frame-oracle.mjs", [3, "[프레임워크] \"셸마다 다른 것을 찍는다\"(Tauri vs Electron)"]],
  ["scripts/e2e/lib/frame-oracle.test.mjs", [3, "[프레임워크] 같은 축"]],
  ["scripts/e2e/frame-verdict.mjs", [2, "[프레임워크] \"셸이 어떻게 찍었든\""]],
