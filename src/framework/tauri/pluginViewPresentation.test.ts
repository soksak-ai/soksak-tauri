...
describe("Tauri plugin renderer RPC surface", () => {
  ...
  it("child toolbar도 같은 탭 주소 아래 공개 측정 DOM으로 투영한다", () => {
    ...
    const projected = projectPluginViewNode(container, {
      label: "b-main-tab-1", realm: "pv-realm", focused: false, realmFocused: false, node: "toolbar", x: 4, y: 8, w: 300, h: 40, rootW: 400, rootH: 260,
  ...
  it("child form node의 종류와 현재 value를 공개 projection에 함께 투영한다", () => {
    const container = document.createElement("div");
    const projected = projectPluginViewNode(container, {
      label: "b-main-tab-1", realm: "pv-realm", focused: false, realmFocused: false, node: "urlbar", x: 4, y: 8, w: 300, h: 40,
