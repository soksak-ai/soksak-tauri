...
export interface AppFramework {
  ...
  /**
   * 이 파일을 **웹뷰가 직접 가져올 수 있는 주소**로 바꾼다.
   *
   * 계약에 두는 이유: 부팅에서 옮기는 양이 곧 기다림인데, 프로세스 경계를 건너는 값은 문자열로
   * 직렬화된다. 실측 2026-08-08 — 플러그인 번들 23.8MB 중 약 15MB 가 그 경계를 건너느라 818ms
   * 를 썼고, 34 건을 한 호출로 묶어도 그대로였다(왕복 수가 아니라 양이 값이었다). 엔진은 같은
   * 파일을 자기 자원 적재 경로로 읽을 수 있고, 그 길에는 직렬화가 없다.
   *
   * 주소를 만드는 방법은 프레임워크마다 다르고 서로를 모른다 — 그래서 여기서 묻는다.
   */
