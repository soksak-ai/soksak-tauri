...
export interface AppFramework {
  ...
  /**
   * 콘텐츠 뷰의 표면이 **DOM 레이아웃을 따라가는가.**
   *
   * 이 하나로 갈리는 것이 크다. 따라가면(DOM 안에 사는 프레임워크) 슬롯이 움직일 때 콘텐츠도
   * 같이 움직인다 — 덮을 것도, 감출 것도, 좌표를 쓸 것도 없다. 안 따라가면(문서 밖 네이티브
   * 자식) 표면은 명시 좌표 쓰기로만 움직이고, 그 쓰기는 컴포지터 애니메이션을 절대 못 따라간다 —
   * 그래서 움직이는 동안 정지 사진으로 덮고 표면을 감추는 장치가 필요하다(slotFreeze).
   *
   * **그 장치는 따라가지 않는 쪽의 것이다.** 기본값으로 세우면 따라가는 프레임워크에서도
   * 표면을 감췄다 드러내게 되고, 그 사이 한 프레임이 배경으로 빈다 — 워크어라운드가 없던
   * 결함을 만든다(실측 2026-08-02: "교체가 끝나고 딱 한 프레임에서 사라졌다 다음 프레임에서
   * 나타난다"). 두 껍데기는 동급이므로, 한쪽의 물리를 코어의 기본으로 두지 않는다.
   *
   * 계약에 두는 이유: 이 사실은 어댑터만 안다. 코어가 추측하면 그 추측이 곧 기본값이 된다.
   */
  readonly surfaceFollowsLayout: boolean;
