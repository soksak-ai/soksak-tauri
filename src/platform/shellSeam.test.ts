...
// 셸 경계 게이트(정적) — 앱 코드는 셸 벤더 SDK 를 몰라야 한다.
//
// 이 게이트가 없으면 경계는 하루 만에 샌다: 새 파일 하나가 편의로 `@tauri-apps` 를 직접
// import 하는 순간 셸은 다시 교체 불가능한 전제가 된다(실측 2026-07-27, 경계 도입 전:
// 벤더를 직접 물던 앱 파일 57개). 규칙은 문서가 아니라 여기서 시행된다.
//
// 허용 구역은 어댑터뿐이다. 새 셸을 붙이는 일은 `src/platform/<shell>.ts` 를 하나 더
...
describe("셸 경계 — 벤더 SDK 는 어댑터만 안다", () => {
  it("어댑터 밖의 앱 코드는 셸 벤더를 직접 import 하지 않는다", () => {
