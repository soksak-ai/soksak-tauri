...
//    호출 자체가 차단된다(자기증식 금지). 유일한 예외 = dev 소스: 개발자가
//    로컬 경로를 직접 지정해 적재한 자기 작업물은 동의 게이트 밖이다(제3자
//    위험 고지 대상이 아님) — 적재 명령(plugin.dev.load) 자체가 danger:"inject"
//    로 원격 위험 정책의 통제를 받으므로 게이트는 적재 시점에 있다.
