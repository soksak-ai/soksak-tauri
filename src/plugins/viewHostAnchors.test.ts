...
//
// 한 값 한 이름: data-tab-id(어휘 표준 — 인스턴스는 탭이다). 옛 data-pane-id 는 소비자 전원 이행 후 제거(2026-07-27).
...
describe("viewHostAnchors — 콘텐츠 뷰 호스트 DOM 앵커 계약", () => {
  ...
  it("콘텐츠 배치(탭 有)는 탭 역참조 앵커(data-tab-id)를 노출", () => {
  ...
  it("사이드바 배치(탭 없음)는 앵커를 노출하지 않는다 — 추종 대상과 혼동 방지", () => {
