...
const VERDICTS: Record<Scope, Record<string, Verdict>> = {
  src: {
    ...
    pty_pane_pid: "tab",
    // ── 새 뜻(칸) — 네이티브 서피스 호스트 가족 ────────────────────────────────
    // 한 칸이 자기 렌더러와 그 안의 네이티브 멤버들을 소유하고 함께 움직인다. 이 이름들이
    // 가리키는 것은 탭 인스턴스가 아니라 **배치의 칸**이다: `logicalPaneId` 가 담는 값이
    // `pan-<n>`(실체5 의 pan)이고, `PaneSurfaceHost` 는 그 칸의 렌더러와 멤버 표면을
    // 소유한다. 이 갈래는 어휘가 정해진 뒤에 지어져 처음부터 새 뜻으로 쓰였다.
    // `panel*` 철자는 명령 표면의 옛 낱말이고 가리키는 것은 같은 칸이다(머리말 ②).
    DomPaneFact: "pane",
    NativePaneFact: "pane",
    PaneCompositionProbe: "pane",
    PaneHostIdentityBinding: "pane",
    PaneRect: "pane",
    PaneResizeCompositionPlane: "pane",
    PaneSurfaceHost: "pane",
    PaneSurfaceHosts: "pane",
    claimPaneSurface: "pane",
    comparePaneNativeContracts: "pane",
    comparePanePresentation: "pane",
    desiredLogicalPaneId: "pane",
    domPanes: "pane",
    explicitPane: "pane",
    exposePaneHostIdentities: "pane",
    focusedPane: "pane",
    installPaneSurfaceHostCommands: "pane",
    logicalPaneId: "pane",
    paneAnchors: "pane",
    paneGreen: "pane",
    paneLayoutContractOf: "pane",
    paneRect: "pane",
    paneRelation: "pane",
    pane_view: "pane",
    panelAtomicMotion: "pane",
    pluginViewPaneHostsStatus: "pane",
    prepare_pane_surface_host_translation: "pane",
    readPaneContract: "pane",
    releasePaneSurface: "pane",
    setLogicalPaneId: "pane",
    set_pane_surface_host_lighting: "pane",
    sharedPaneHost: "pane",
    syncPaneFrame: "pane",
    webview_pane_bounds: "pane",
    webview_pane_group: "pane",
    webview_pane_hosts: "pane",
    webview_pane_lighting: "pane",
    webview_pane_member_bounds: "pane",
    webview_pane_transition_prepare: "pane",
  },
  "frameworks/tauri": {
    ...
    // ── 새 뜻(칸) — 위 src 가족의 Rust 쪽 몸 ───────────────────────────────────
    // 같은 사실이 두 언어에 산다. 스코프별로 따로 판정한다(합치면 한쪽 미판정이 가려진다).
    Pane: "pane",
    PaneMemberLayoutContract: "pane",
    PaneSurfaceHost: "pane",
    PaneSurfaceRecord: "pane",
    SoksakPanePresentationTraceTarget: "pane",
    _pane: "pane",
    capture_pane_presentations: "pane",
    configure_pane_member_resize: "pane",
    detach_surface_from_pane: "pane",
    group_pane_surface_host: "pane",
    grouped_pane_members_inherit_parent_resize_in_the_same_appkit_epoch: "pane",
    keeps_member_insets_while_the_pane_host_resizes: "pane",
    pane_frame: "pane",
    pane_group_converts_declared_members_from_their_adapter_parent: "pane",
    pane_host: "pane",
    pane_layout_tests: "pane",
    pane_member_availability_tests: "pane",
    pane_member_available: "pane",
    pane_members: "pane",
    pane_motion: "pane",
    pane_motion_and_trace_share_one_media_unix_clock_anchor: "pane",
    pane_presentation_trace_is_a_finite_native_display_link_contract: "pane",
    pane_surface_host_state: "pane",
    pane_topology_observes_external_hosts_from_the_public_host_registry: "pane",
    pane_view: "pane",
    panes: "pane",
    prepare_pane_surface_host_translation: "pane",
    presentation_generation_belongs_to_surface_identity_not_pane_grouping: "pane",
    project_panes: "pane",
    resize_pane_children: "pane",
    resize_pane_surface_hosts: "pane",
    resize_pane_surface_hosts_for_viewport: "pane",
    set_pane_surface_host_bounds: "pane",
    set_pane_surface_member_bounds: "pane",
    surface_pane: "pane",
    surface_panes: "pane",
    webview_pane_bounds: "pane",
    webview_pane_group: "pane",
