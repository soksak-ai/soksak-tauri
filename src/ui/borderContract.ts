...
export const BORDER_RULES: readonly BorderRule[] = [
  ...
  // 좌측 사이드바와 본문 사이의 세로 경계 — **사이드바가 right 를 소유**한다.
  // §B2 가 이미 "좌측 크롬=right" 로 명령해 둔 변인데 CSS 는 한 번도 긋지 않았고, 이 표에도
  // 없어 검증기가 묻지 않았다(실측 2026-07-31: ui.expect ".sidebar-left" → rules: []).
  // 표에 없으면 위반이 위반으로 드러나지 않는다 — 요소가 있는데 규칙이 0인 것은 통과가
  // 아니라 무판정이다. 리사이저는 이 선을 못 그린다(§B6): 폭 0 오버레이라 hover 때만
  // 색이 뜨고, 평상시엔 경계가 아무에게도 없었다.
  // 톤은 bd-soft — 본문과 사이드바는 같은 패널 영역 안(둘 다 --pane-inset 을 먹는다)이라
  // 앱 크롬 경계(레일 right, 톤 bd)가 아니라 패널 내부선이다(§B4).
  {
    id: "left-sidebar-body-edge",
    selector: ".sidebar-left",
    kind: "edges",
    edges: { right: "bd-soft" },
    note: "§B2 — 좌측 크롬이 본문 쪽 변(right) 소유, §B4 패널 내부선",
  },
