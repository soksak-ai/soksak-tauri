...
// 세 소비자 계층에 에지로 알린다: ① 플러그인 events(layout.resize-gesture) ② 선택된
// 프레임워크의 네이티브 배치 전환
...
//            Tauri는 같은 최종 frame·duration·curve를 AppKit에 한 번 넘긴다.
//   resize = 표면 크기가 프레임마다 변한다(디바이더·폭 드래그). 코어는 이 사실만 공개한다.
//            DOM 자식은 부모와 함께 크기가 바뀌고, 문서 밖 표면의 합성 정책은 해당 어댑터가
//            맡는다(Tauri는 이벤트 기반 bounds 정착, Electron은 추가 동작 없음).
// 페이로드 { active, kinds }: kinds = 그 순간 활성인 종별들. active 인 채 종별 구성이
// 바뀌면 active:true 를 재발화한다 — 소비자는 재평가한다(예: 주행 중 디바이더 개입).
...
// 소비자는 합집합의 표면만 전환한다. 관련 없는 표면은 남의 스왑에 native frame 명령을
// 받지 않으며 위상 내내 그대로 남는다.
...
// 채널은 종별 구성 변화도 받는다(active:true 재발화 — 어댑터 재평가의 근거).
...
function syncEmit(): void {
  ...
  // 플러그인 채널은 공통 사실(active·kinds)만 싣는다. 표면 범위는 프레임워크 내부 구독자가
  // 공개 DOM 주소와 scope를 대조한다.
  ...
  // (네이티브 배치)가 kinds·scope 재평가를 해야 하므로 플러그인 채널과 같은 조건으로 부른다.
