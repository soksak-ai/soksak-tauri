...
// ⊆ 이 창 스토어의 브라우저 뷰 집합. 생성/파괴는 BrowserView 컴포넌트(레거시) 또는 브라우저 플러그인
// (콘텐츠 뷰)의 수명주기가 담당하지만, 비동기 생성과 빠른 닫기/이동이 겹치면 고아 웹뷰(아무도 못 닫는
// 유령 창)가 생길 수 있다 — 레이아웃이 바뀔 때마다(이벤트 기반, 폴링 없음) 불변식을 검증·회수한다.
// label 은 창 네임스페이스(webviewLabels 단일 진실)라 browser_list(앱 전역)를 이 창 접두사로 걸러
// *자기 창* 것만 회수한다(다른 창 것 종료 금지). 브라우저 뷰 "집합"이 변한 경우에만 네이티브
// 질의(browser_list) — 드래그 등 무관한 store 쓰기는 문자열 비교 한 번으로 끝난다(docs/PERFORMANCE.md
// 원칙 5).
//
// [중요] 브라우저는 두 형태로 존재한다: 레거시 코어 뷰(kind:"browser")와 브라우저 플러그인 콘텐츠
// 뷰(kind:"plugin", pluginId=soksak-plugin-browser). 둘 다 `browserLabel(view.id)` 로 같은 webview
// label 을 만든다(app.webview.label = browserLabel). 한 형태만 live 로 보면 다른 형태의 살아있는
// webview 를 고아로 오인해 회수해 버린다(레거시만 보던 시절의 버그) — 두 형태를 모두 live 로 센다.
    ...
    for (const c of t.contents) {
      for (const g of allGroups(c.layout)) {
        for (const v of g.views) {
          // 레거시 브라우저 뷰.
          if (v.kind === "browser") live.add(labelOf(v.id));
          // 브라우저 플러그인 콘텐츠 뷰 — 같은 label 스킴으로 webview 소유.
          else if (v.kind === "plugin" && v.pluginId === WEBVIEW_OWNER_PLUGIN_ID)
