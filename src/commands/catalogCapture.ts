...
export function registerCaptureCatalog(): void {
  register("window.snapshot", {
    description:
      "Capture the window contents to a PNG. Captures even when fully occluded by other apps (occlusion detection is temporarily disabled during capture). Includes WebGL terminal. Parent folder is created automatically. Cropping and saving compose freely: rect (CSS px, window coords — same space as ui.measure) or node (an exposed address from ui.tree — captures exactly that tab/panel/element) selects the region, and path saves it while base64:true returns it inline. With neither path nor base64, a cropped capture still returns inline.",
    ...
    params: {
      ...
      node: {
        type: "string",
        description:
          "Exposed address (ui.tree) to capture — its rect is measured for you. Use this to capture one tab, panel, or element without computing coordinates.",
      },
    },
    returns:
      "{ saved, media:{kind,path} } when path is given (cropped or full) | { media:{kind:'image/png',base64} } otherwise",
    message: (d) =>
      d.saved
    ...
    handler: async (p) => {
      ...
      if (typeof p.node === "string" && p.node) {
        const found = resolveExposed(p.node);
        ...
        if (cropped.w < 1 || cropped.h < 1) {
          return {
            ...
            message: `노드가 화면에 크기를 갖지 않습니다(${Math.round(r.width)}x${Math.round(r.height)}): ${p.node}`,
          };
        }
        ...
        if (
          ...
          return {
            ...
            message: `노드가 화면 밖입니다(x=${cropped.x}, y=${cropped.y} · 뷰포트 ${vw}x${vh}) — 비활성 슬롯은 창 밖으로 파킹됩니다. 먼저 그 스페이스/탭을 활성화하세요: ${p.node}`,
          };
        }
      ...
      if (rect || p.base64) {
        ...
        if (outPath) {
          ...
          return { saved: w.path, bytes: w.bytes, media: { kind: "image/png", path: w.path } };
        }
