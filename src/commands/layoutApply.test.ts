// layout.apply 유닛 — 시트/패널을 새로 짓는 위계(1차 시트·2차 분할)와 미설치 프로그램 건너뛰기,
...
describe("layout.apply", () => {
  it("preset dev — 브라우저가 있으면 터미널+브라우저 2 패널을 한 시트로 짓는다", async () => {
    unregBrowser = registerBrowser();
    const r = await execute("layout.apply", { preset: "dev" }, {});
    expect(r.ok).toBe(true);
    const sheets = (r.data as { sheets: { title: string; panels: { program: string }[] }[] }).sheets;
    expect(sheets).toHaveLength(1);
    expect(sheets[0].title).toBe("dev");
    expect(sheets[0].panels.map((p) => p.program)).toEqual(["terminal", "browser"]);
  ...
  it("preset dev — 브라우저가 없으면 그 패널을 건너뛰고 skipped 에 사유를 담는다", async () => {
    ...
    const data = r.data as {
      sheets: { panels: { program: string }[] }[];
      skipped?: { program: string; reason: string }[];
    };
    expect(data.sheets[0].panels.map((p) => p.program)).toEqual(["terminal"]);
  ...
  it("기본형 문법 — 값 하나(dev)를 preset 위치 인자로 받는다", async () => {
    ...
    expect(r.ok).toBe(true);
    expect((r.data as { sheets: unknown[] }).sheets).toHaveLength(1);
  });

  it("preset facets — sheets 인자대로 이름 붙인 시트 묶음을 짓는다", async () => {
    ...
    expect(r.ok).toBe(true);
    const sheets = (r.data as { sheets: { title: string; panels: unknown[] }[] }).sheets;
    expect(sheets.map((s) => s.title)).toEqual(["a", "b"]);
    expect(sheets[0].panels).toHaveLength(1);
    expect(sheets[1].panels).toHaveLength(2);
    // 2차 패널은 서로 다른 패널 id 다(분할 생성).
    const b = sheets[1].panels as { panelId: string }[];
    expect(b[0].panelId).not.toBe(b[1].panelId);
  });

  it("preset facets — sheets 를 빠뜨리면 INVALID_PARAMS 로 답한다", async () => {
    const r = await execute("layout.apply", { preset: "facets" }, {});
    expect(r.ok).toBe(false);
    expect(r.code).toBe("INVALID_PARAMS");
  });

  it("기존 시트를 파괴하지 않는다 — 새 시트를 더한다", async () => {
    const before = firstProject().contents.length; // 부트 시트 1개
  ...
  it("hint 는 상한 3개를 넘지 않고, 성공 시 제시를 붙인다", async () => {
    ...
    // 첫 시트로 전환하는 수를 제시한다(실제 sheetId 로 렌더).
    const first = (r.data as { sheets: { sheetId: string }[] }).sheets[0].sheetId;
    expect(r.hint!.some((h) => h.cmd === `sok sheet.activate ${first}`)).toBe(true);
  });
