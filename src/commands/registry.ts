...
export interface CommandSpec {
  ...
  // 성공 응답 형태 설명(매뉴얼용).
  returns: string;
  // 이 명령의 답이 **창-지역인가**. 기본은 창-지역(안 적으면 그렇다) — 안전한 쪽이 기본이다.
  //
  // `false` = 창이 아니라 **주인**(저장소·데몬·프로세스·네트워크)이 답을 정한다. 그러면 어느
  // 창에서 돌든 같은 답이고, 같은 이름을 든 창이 여럿일 때 **한 번만** 돌면 된다.
  //
  // 왜 필요한가: 한 홈에 두 앱이 뜨면 `main` 같은 이름을 둘이 든다(오케스트레이터는 앱마다
  // 하나다). 그때 그 이름으로 온 명령을 전부에게 보내면 창-지역 명령은 그게 맞지만, 주인이
  // 답하는 명령은 **같은 일이 두 번 돈다**(실측 2026-08-01: `data.kv.set` 이 두 프로세스에서
  // 각각 실행됐다). 답이 어디서 오는지는 명령 자신이 안다 — 그래서 여기 적는다.
  //
  // 이 선언은 **파생 가능하다**: 핸들러가 창-지역 상태(useSessions·document·window.…)를 만지면
  // 창-지역이다. 게이트가 소스에서 다시 세어 선언과 대조한다(owner-answer-scan).
