...
const ALLOWED: { file: string; mark: string; event: string; why: string }[] = [
  ...
  { file: "src/commands/catalogSettings.ts", mark: "const sleep = (ms: number) => new Promise((r) => setTimeout(r, ms));", event: "caller-specified", why: "sleep 헬퍼(테마 스캔)" },
  { file: "src/commands/catalogSettings.ts", mark: "await sleep(settleMs);", event: "caller-specified", why: "settleMs 파라미터" },
