...
fn validate_peer_identity(reply: &Value, expected: &str) -> Result<(), String> {
    let actual = reply
        ...
        .unwrap_or("<missing>");
    let same_home = reply.get("identity").and_then(Value::as_str).is_some()
        && soksak_core::identity::core_build_for_identifier(actual)
            == soksak_core::identity::core_build_for_identifier(expected);
    if same_home {
        Ok(())
    } else {
        Err(format!(
