// soksak-sidecar-chromium — Chromium 엔진 사이드카의 C ABI 표면(soksak-engine-abi@1).
...
// 호스트가 메인스레드를 보장한다. reply 버퍼는 모듈이 할당하고 호스트가 soksak_engine_free 로 돌려준다.
...
unsafe impl Sync for SoksakSidecarAbi {}
...
static ABI: SoksakSidecarAbi = SoksakSidecarAbi {
    abi: HOST_ABI_VERSION,
    model: c"engine".as_ptr(),
    interface: c"soksak-engine-chromium@1".as_ptr(),
    version: c"0.1.0".as_ptr(),
};

#[no_mangle]
pub extern "C" fn soksak_sidecar_abi() -> *const SoksakSidecarAbi {
    &ABI
}
...
#[no_mangle]
pub extern "C" fn soksak_engine_init(
    host: *const SoksakEngineHost,
    cfg_json: *const u8,
    cfg_len: usize,
) -> i32 {
...
#[no_mangle]
pub extern "C" fn soksak_engine_shutdown() {
    let _ = std::panic::catch_unwind(engine::shutdown_engine);
}

// ── message: 불투명 요청 디스패치(soksak-engine-chromium@1 프로토콜) ─────────────────────────
...
#[no_mangle]
pub extern "C" fn soksak_engine_message(
    req: *const u8,
    len: usize,
    surface: usize,
    reply: *mut SoksakBuf,
...
#[no_mangle]
pub extern "C" fn soksak_engine_notify(evt: *const u8, len: usize) {
...
#[no_mangle]
pub extern "C" fn soksak_engine_free(buf: SoksakBuf) {
    let _ = std::panic::catch_unwind(|| {
        if !buf.ptr.is_null() {
