//! 자식 프로세스 — 띄우고, 흘리고, 거둔다. **한 벌**.
//!
//! 프레임워크 핸들을 받지 않는다: 출구는 계약(StreamSink·ExitSink), 시크릿도 계약
//! (SecretSource), 정체성은 인자다. 앱과 헬퍼가 같은 규칙으로 자식을 띄운다 — 두 벌이면
//! 같은 명령이 프로세스마다 다른 env 로 뜨고, 그 차이는 "이쪽에서만 안 된다"로 나타난다.
...
use crate::secret_env::{resolve_secret_env, SecretSource};
...
impl ProcessManager {
    /// 이 창 앞으로 남은 자식을 거둔다 — 창은 **라벨 문자열**로 온다(창 객체를 쥘 필요가 없다).
    /// 반환 = 거두기 전 그 창의 자식 수.
    pub fn reclaim_window(&self, label: &str) -> u32 {
        let before = self
            .sessions
            .lock()
            .map(|s| s.values().filter(|x| x.window == label).count())
            .unwrap_or(0) as u32;
        self.kill_by_window(label);
        before
    }

    /// 이 자식의 stdin 에 쓴다. 없는 세션·닫힌 stdin 은 이름을 달고 실패한다 — 조용히 삼키면
    /// 부르는 쪽은 "입력이 사라진다"만 본다.
    ...
    /// stdin 만 닫는다(자식은 계속). stdin 을 read_to_end 하는 자식은 이것 없이는 영원히
    /// 블록한다. 이미 닫혔으면 no-op(멱등).
    ...
    /// 지금 살아 있는 자식들. 코어가 프로세스를 쥐고 목록을 안 내놓으면 회수에 실패한
    /// 자식(고아)이 밖에서 보이지 않는다.
    ...
    /// 이 자식을 거둔다(group 스폰이면 트리 전체). 없는 id 는 no-op — 이미 끝난 것을 다시
    /// 거두는 것은 오류가 아니다.
...
pub fn resolve_sidecar_cmd(identity: &Identity, cmd: &str) -> Result<String, String> {
...
pub fn detached_gate(cmd: &str, detached: bool) -> Result<(), String> {
...
pub fn spawn_child<O, E, X>(
    identity: &Identity,
    // 상속될 env 의 **키 목록**은 프로세스의 사실이다 — 여기서 읽으면 같은 코드가 프로세스마다
    // 다른 것을 지운다(no_framework 게이트가 그 철자를 막는다). 값은 필요 없다: 지울 이름만 본다.
    ...
    X: ExitSink,
{
    ...
    let resolved_secrets = resolve_secret_env(secrets, ns.as_deref(), &secret_env)?;
    ...
    for k in internal_soksak_keys(inherited_keys.iter().cloned()) {
        c.env_remove(k);
    }
...
mod tests {
    use super::*;
    use crate::identity::Identity;
    use crate::secret_env::NoSecrets;
    ...
    fn run_probe(identity: &Identity, script: &str) -> (Vec<u8>, i32) {
        ...
        // 이 probe 는 시크릿을 쓰지 않는다 — 볼트 없는 호스트로도 스폰이 서는지가 요점이다.
        let secrets_state = NoSecrets;
        let id = spawn_child(
            identity,
            // 검사는 상속 env 를 명시한다 — 프로세스 환경에 기대면 같은 검사가 기계마다 다르다.
            &[],
            SpawnRequest {
                cmd: "/bin/sh".into(),
