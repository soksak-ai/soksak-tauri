...
/// identifier → OS 키체인 서비스명. **홈과 같은 축(env)이다.**
///
/// 볼트는 홈에 산다(`<home>/secrets.vault`). 그 열쇠가 프레임워크로 갈리면 파일과 열쇠가 서로
/// 다른 축이 되고, 홈을 공유해도 둘째 프레임워크는 그 볼트를 못 연다. 못 여는 것으로 끝나지도
/// 않는다 — 빈 볼트를 새로 만드는 경로로 새면 앱은 "시크릿이 비었다"를 답하고, 그것은 오류로
/// 보이지 않는다.
///
/// 프레임워크 축을 뺀 모양은 옛 identifier 와 같다(`com.soksak.<env>`). 그래서 프레임워크 축이
/// 붙기 전에 만들어진 볼트가 그대로 열린다 — 이관 스크립트가 필요 없다.
///
/// 이것이 격리의 전부는 아니다. macOS 키체인 항목의 접근 권한은 **바이너리 단위**라, 서비스명이
/// 같아도 다른 실행물은 사용자 확인을 거친다(거절이 아니라 확인이다).
pub fn keychain_service_for_identifier(identifier: &str) -> String {
    let segs: Vec<&str> = identifier.split('.').filter(|s| !s.is_empty()).collect();
    // 접두를 하드코딩하지 않는다 — identifier 에서 **프레임워크 세그먼트만** 뺀다. 하드코딩하면
    // 접두가 바뀌는 순간 규칙이 두 벌이 되고, 그중 하나가 조용히 남의 KEK 를 연다.
    if axes_of_identifier(identifier).0.is_none() {
        return identifier.to_string();
    }
    let mut kept: Vec<&str> = segs.clone();
    kept.remove(segs.len() - 2);
    kept.join(".")
}
