...
//
// 프레임워크를 어댑터 뒤로 보낸 것과 같은 이유로 출구도 어댑터 뒤에 둔다. 크로싱의 실체는
// 프레임워크마다 다르다 — Tauri 는 웹뷰 IPC 핸들(`Channel`), 헬퍼는 소켓 위의 토큰이다.
// 어느 쪽도 Serialize 가 아니거나 프로세스 경계를 못 넘으므로, 그 타입이 시그니처에 박혀
// 있으면 그 핸들러는 그 프로세스를 떠날 수 없다(실측 분류 2026-07-27: 옮길 수 있는 7건 중
// 3건이 오직 이것 때문에 묶였다).
//
// 계약이 코어에 있는 이유: 같은 세션 코드가 두 프로세스에서 돌고, 출구만 다르다.
//
// 계약은 두 줄이다:
//   ① 배치 하나를 소비자에게 건넨다.
...
pub trait ExitSink: Send + 'static {
