//! 범용 UDP — 임의 host:port 로 단발 송신, 또는 한 소켓에서 요청-응답 수집.
//!
//! webview JS 는 raw UDP 를 열 수 없어(보안) 코어가 제공하는 범용 기능이다(특정 용도 락인 0 —
//! Wake-on-LAN·디스커버리·IoT 등 무엇이든). 1회용 소켓(0.0.0.0:0 바인드 → 드롭)이라 세션도
//! 관리 상태도 없다 — 그래서 이 로직은 어느 프로세스에서 돌든 같은 답을 낸다.
//!
//! 다만 **OS 신원은 프로세스를 따라간다**: macOS 의 로컬 네트워크 프라이버시 게이트는
//! responsible process(앱 번들 신원)에 붙는다. cored 가 별도 번들 신원으로 돌면 브로드캐스트·
//! 멀티캐스트 응답이 조용히 사라질 수 있다(send_to 는 성공하고 응답만 안 온다). 코드는
//! 이동 가능하지만 배치는 같은 번들 안이어야 한다 — 적대적 감사(2026-07-27)가 짚은 조건.
