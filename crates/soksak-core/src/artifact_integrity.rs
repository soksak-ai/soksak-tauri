//! **설치 실행물**의 무결성 — 해시 검증, 부분/깨진 설치 관찰, 화이트리스트 안의 stale 제거.
//!
//! 저장소 무결성이 아니다. 그쪽은 `store_integrity`(SQLite 손상 게이트·REINDEX 치유)이고,
//! 한때 두 모듈이 똑같이 `integrity` 라는 이름을 지고 있었다 — 같은 이름이 두 뜻을 지면
//! 읽는 사람이 남의 규칙을 이 규칙으로 읽는다.
//!
