...
sok plugin.dev.load '{"path":"/path/to/soksak-shark"}'   # 개발 로드
```
...
| 명령 | 설명 |
|---|---|
| `plugin.soksak-shark.toggle` | 상어 보이기/숨기기(숨기면 rAF 완전 정지) |
| `plugin.soksak-shark.mode '{"reactive":true}'` | 반응형 on/off — on 이면 커서가 가까이 오면 슬쩍 피한다(off=순수 장식, 통과) |
| `plugin.soksak-shark.speed '{"liveliness":1.5}'` | 활기 배수 0.3~3 (이동·헤엄 전반 속도) |
