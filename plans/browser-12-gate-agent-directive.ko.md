...
- 건너뜀의 주인은 원장이 답한다. 궤적의 시각 간격만 보면 관측 콜백이 밀려 표본을 못 낸 것과 합성기가 프레임을 실제로 건너뛴 것이 같은 모양으로 나온다. 앱의 원장이 자기가 놓친 콜백 수를 스스로 답하므로(`callbackIntervalsSkipped` — Tauri `presentation_trace.rs`, Electron `presentationLedger.ts` 둘 다) 그 답으로 가른다. 안 답한 관측자를 "안 놓쳤다"로 읽지 않는다. `slot` 열은 DOM 관측이라 네이티브 원장의 사실이 아니고, 그 열의 주인은 실측에서 파생한다 — 표시 callback이 빈 구간에 recorder 표본이 있었으면 관측 경로는 살아 있었고 그 구멍은 표시가 실제로 건너뛴 것이다.
- 한 전이의 못 잼이 나머지 전이를 데려가지 않는다. 표시 궤적 무장은 첫 표시를 기다리고(`FIRST_DISPLAY_TIMEOUT`), 그 안에 안 오면 그 전이는 못 잰 것이지 제품이 틀린 것이 아니다. 기록하고 계속 재며(`presentation-arm-ledger`) 판정은 마지막에 한 번 한다 — 무장을 한 번이라도 놓친 실행은 그 칸을 이름과 함께 `blocked`로 답한다.
- 침묵과 미선언은 다른 답이다. 앱이 어떤 축을 아직 안 내는 실행물일 수 있으므로, 활성 플러그인이 선언을 안 답하면 "아무도 구현하지 않는다"로 읽지 않는다. 비활성·오류 플러그인은 먼저 거른다 — 남의 상태 때문에 이 판정이 죽으면 안 된다.
- 적어 둔 계약은 읽혀야 한다. 계약에 축을 세우고 판정도 그것을 읽게 했는데 부르는 쪽이 그 계약을 한 번도 안 부르면, 단위 테스트는 전부 통과하고 라이브에서만 모든 칸이 "선언 없음"으로 닫힌다. 배선은 소스에서 셀 수 있어야 한다 — 실행해야만 보이는 배선은 배선이 아니다.
- 통과가 아닌 판정은 왜인지 함께 낸다. 잰 어긋남은 `evidence`에, 못 잼의 사유는 `reason`에 산다. 한쪽만 읽으면 `blocked`가 이유 없이 찍히고 그 칸은 고칠 자리를 가리키지 못한다. 둘 다 없으면 그 사실 자체를 이름으로 낸다.
- 안 답한 자리·다른 모양·틀린 값은 셋 다 다른 답이다. 자기 구현을 가진 참가자는 코어 원장이 답하는 값을 다 답하지 못하고, 계약이 그 자리를 `null`로 두는 것은 지어내지 않기 위해서다. 안 답함은 못 잼(사이드카가 채울 자리), 다른 모양은 계약이 옮기고(숫자 id를 이름으로 — 판정 기준을 넓히면 "무엇이 정체인가"가 흐려진다), 틀린 값은 red다(`0`·`""`·`NaN`도 답한 것이므로 안 답함으로 도피하지 않는다). 판별은 한 자리가 든다(`isUnanswered`) — 자리마다 손으로 쓰면 반드시 하나가 빠진다. 껍데기만 보면 안쪽의 `null`이 새므로 사각형은 축까지 본다.
- 원인 위에 증상을 겹쳐 적지 않는다. 안 답한 정체로 유일성을 재면 안 답한 둘이 "중복"이 되고, 안 답한 값으로 순서를 비교하면 없는 사실이 번진다. 원인을 이름으로 냈으면 그 아래 판정은 건너뛴다.
- 사람용 녹화의 분량은 판정을 바꾸지 않는다. 판정은 수치 영수증이 소유하므로 프레임 수는 어느 칸의 답도 바꾸지 않는다 — 실행이 긴 환경에서 판정을 끝까지 받으려면 부르는 쪽이 그 분량을 정할 수 있어야 한다(`SLOT_FREEZE_RECORD_FRAMES`). 기본은 지금 값이고 선언은 그보다 크지 못하며, 게이트가 판정 축이 그 값을 읽지 않는지 센다.
