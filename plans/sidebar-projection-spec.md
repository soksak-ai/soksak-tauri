...
- **F7 선택적 근접 배치.** 전역 설정 `가까운 쪽으로 붙이기`가 켜지고 포커스 패널의 자체 왼쪽 선이 깨끗하지 않으면, 레일을 더 앞 선에 두는 대신 그 패널을 같은 가로 분할의 직접 형제와 **화면에서만** 교환해 깨끗한 앞 선에 붙인다. 저장된 분할 트리·크기 배열은 바꾸지 않는다. 이미 깨끗한 선의 패널이나 다른 비영향 패널로 포커스가 이동하면 정본 트리에서 다시 투영되어 원래 배열로 즉시 복귀한다. subtree 이동이나 세로 구조 변경은 하지 않으며, 교환 결과도 깨끗한 선이 아니면 F6을 적용한다. 기본값은 끔(원래 배열 유지)이다.
- **F8 최대화.** 기능 탭 더블클릭은 결부 사이드바를 0번 깨끗한 선에 유지한 채 해당 기능 패널을 나머지 콘텐츠 평면 전체로 최대화하여 `[사이드바 | 최대화 기능창]`을 만든다. 최대화는 기존 내부 station을 FULL_RECT의 출발 기하로 재사용하지 않는 원자적 평면 전환이다. 다시 더블클릭하거나 복원하면 저장된 분할 트리와 F7 투영을 재적용한다.
- **F9 공개 투명성.** 저장 정본 트리와 현재 표시 트리를 구분해 공개하고, 투영 종류·적용 여부·포커스 패널·교환 패널을 상태면에 노출한다. 레일·그리드·패널·뷰 슬롯은 주소 가능한 DOM 노드이며 선언된 `data-*`는 커맨드로 읽을 수 있다. 설정 변경·패널 포커스·최대화·복원·레일 배치는 공개 커맨드와 상태면으로 검증한다. 플러그인 식별자나 private DOM·store를 판별해 분기하지 않는다.
- **F10 최대화 표면 독점.** 최대화 대상이 아닌 뷰 슬롯은 상태 복원을 위해 mount를 유지하되 DOM·GPU composition에서 제외한다. 복원 이벤트에서 전체 표시 표면을 재배치한다. 이 규칙은 플러그인 종류가 아닌 최대화 상태만으로 결정한다.
- **F11 결부 관계 표면.** 열린 좌 레일과 결부 패널이 실제로 맞닿을 때 두 rect의 합집합을 하나의 관계 표면으로 표시한다. 전체 높이 레일과 결부 패널만 포함하고 사이의 다른 패널·비결부 패널은 덮지 않는다. 표면은 레이아웃을 점유하지 않고 입력을 받지 않으며, 저장 트리·분할 크기·hit-test를 바꾸지 않는다. 레일과 패널이 떨어진 PIN 상태에는 거짓 연결선이나 채움을 만들지 않는다. 결부 view·panel·실제 연결 여부와 관계 DOM을 공개 상태면에 노출한다.
- **F12 테마·픽셀·실시간 계약.** 관계 표면은 paneStyle의 보더·gap·radius를 재사용하지 않는 독립 벡터 경로다. 테마는 `relation`의 stroke·fill·strokeWidth·radius·label을 선언하며, 구 테마는 블록 전체 기본값으로 승격한다. 새 블록의 부분 선언은 거부한다. 경로 좌표는 레일·패널과 같은 그리드 사상에서 얻고 창 크기·분할 resize·split·merge·FLOW 이동에 이벤트로 즉시 재계산한다. 구현은 단일 SVG path와 단일 ResizeObserver를 사용하며 폴링·프레임별 DOM 측정·패널별 복제를 금지한다.
...
4. **영역 인계**: FLOW 스테이션 변경은 레일 하나를 pane 위로 운반하는 동작이 아니라, 출발 복도를 닫고 도착 복도를 여는 그리드 재배치다. 최종 공간 배치를 먼저 확정하고 이동 영향을 받는 pane은 출발점과의 차이를 340ms compositor `translate`로 0까지 보간하는 FLIP 주행을 한다. 도착 레일과 새 투영 내용은 시작 프레임부터 최종선의 그리드 바닥에 존재하며, pane이 비켜 도착 복도가 넓어지는 만큼 드러난다. 출발 레일과 이전 투영 내용은 출발선에 유지되며, pane이 복도를 좁혀 완전히 가린 뒤 제거한다. 두 레일 표상은 전환 중에만 공존하고 종료 후 도착 표상 하나만 남는다. `left`·`width`의 프레임별 보간, pane 위 오버레이, 레일 z축 승격, CSS 등장 효과, 페이드와 교차 블렌딩은 금지한다. 드래그 중에는 애니메이션을 끊어 손을 즉시 따른다.
