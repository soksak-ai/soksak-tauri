...
**A2. 흡수와 투영.** 도구는 사이드바를 내부에 렌더링하지 않는다. 사이드바는 글로벌 레일에 투영되고, 결부(binding)된 콘텐츠 뷰가 바뀌면 레일 내용이 그 뷰의 선언으로 전환된다. 좌·우 대칭 규칙이다.

**A3. 3화면 정형.** 창의 레이아웃은 `[좌 레일 | 콘텐츠 | 우 레일]`로 고정한다. 분할은 콘텐츠에서만 일어나며 재귀·무제한이다. 레일 안에서 콘텐츠 분할을 만들지 않는다.
...
**A8. 결부는 명시 상태.** 좌–중앙–우 연결은 DOM 포커스에서 파생하지 않는다.
- 결부의 정본은 세션 상태의 활성 체인이다: `activeProject → activeContent(스페이스) → activeGroup → activeView`. 이 체인은 이미 명시 상태이며 작성자는 코어뿐이다. projection은 이 체인 위의 확장(해소 결과·핀·이력)이지 이중진실이 아니다.
- **결부 대상 = 활성 콘텐츠 뷰(View)다. 그룹이 아니다.** 그룹은 다중 탭 스택이므로, 그룹 내 활성 탭 전환도 결부 변경이다.
- 입력 포커스와 결부를 분리한다. 레일 상호작용은 결부를 바꾸지 않는다.
- 결부 뷰가 닫히면 focusHistory로 승계한다(R6).
...
| 결부(binding) | 레일이 어느 콘텐츠 뷰의 선언을 투영하는가의 관계. 활성 체인의 말단(활성 뷰)이 결정 |
| 결부 뷰 | 활성 프로젝트의 활성 스페이스의 활성 그룹의 활성 콘텐츠 뷰(View). 결부의 단위 |
...
**스코프 = 창 × 프로젝트.** 셸이 프로젝트별로 레일 인스턴스를 유지하는 구조와 일치한다. 프로젝트/스페이스 전환은 그 컨테이너의 활성 체인으로 결부를 재유도한다 — 별도 재결부 쓰기가 필요 없다(A8: 결부 정본 = 활성 체인).
...
- `projection.changed`: 결부·슬롯·핀 변경 시. **그룹 내 활성 탭 전환도 결부 변경이므로 발화한다.** 레일 렌더는 이 이벤트(상태 구독)로만 반응한다.
...
- `focusHistory`: 세션 로컬(복원 대상 아님 — 복원 직후는 활성 체인 단독으로 시작).

**안정 정체성.** 복원이 콘텐츠 뷰 레코드를 보존하는 한 per-view 상태는 레코드에 실려 함께 복원된다. 복원 후 상태가 다른 뷰에 붙거나 유실되는 것은 결함이다.

## 5. 동작 규칙

**R1 투영 전환.** 결부 뷰 변경 시(그룹 간 이동·그룹 내 탭 전환 모두) 새 결부 뷰의 선언을 해소해 레일 슬롯을 갱신한다. 새 instanceKey가 기존과 같으면 그 슬롯은 전환하지 않는다(상태·스크롤 보존). 투영 뷰는 keep-alive(display 토글)로 유지한다 — 상한·퇴거는 구현 재량이되 구조 상태 보존 계약(§4.5)은 유지한다.
...
- 핀 항목 = ref다(instanceKey가 아니다). **핀 가능한 것은 shared 참조와 상주형 뷰뿐이다** — per-view 참조는 핀 불가(명령·UI 모두 거부).
...
**R5 강등.** 참조 해소 실패(계약 미구현·비활성·consumes 위반) 또는 선언 부재 시 해당 슬롯은 degraded 상태로 빈 슬롯+안내를 렌더한다. 다른 슬롯과 핀은 영향받지 않는다. 원인 해소(플러그인 적재 등) 시 상태 유실 없이 승격된다.

**R6 승계.** 결부 뷰가 닫히면 **같은 스페이스 안의** focusHistory 최근 생존 뷰로 결부를 승계한다. 스페이스에 뷰가 없으면 결부는 null이고 투영 슬롯은 비우고 핀만 남는다. 승계와 intent 배치는 활성 프로젝트·활성 스페이스의 가시 문맥으로 한정된다 — 숨은 컨테이너의 패널에 결부하거나 배치하지 않는다.
...
**R9 복원 완전성.** 콜드 재기동 후 projection은 종료 시점과 동형으로 재현된다: 결부(활성 체인), 레일 슬롯 구성, 각 슬롯의 instanceKey 연결, 핀, 투영 뷰의 구조 상태, 뷰별 selection 전부. 웜 복원은 여기에 연속성(전환·깜빡임 없음)을 더한다. 복원 시점에 참조 플러그인이 아직 미적재면 슬롯은 강등(R5)으로 시작하고 적재 완료 시 상태 유실 없이 승격된다. 검증 기준은 기존 규율을 따른다: 콜드 1회만의 GREEN은 인정하지 않는다 — 연속 웜 GREEN까지 멱등이어야 한다.
...
1. 결부 축 재정의 — 결부 대상=콘텐츠 뷰(탭 전환도 결부 변경), 스코프=창×프로젝트, 정본=세션 활성 체인 (critical×4: 창 단위 상태가 프로젝트/스페이스/다중 탭 계층과 충돌).
