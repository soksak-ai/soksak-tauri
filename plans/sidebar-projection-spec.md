...
4. **실이동**: 사이드바의 이동은 hide→show 연출이 아니라 실좌표 주행이다 — FLOW 스테이션 변경 시 레일 프레임이 transition 으로 실제 이동하고, 창(pane) 뒤로 지나가면 충분하다. 콘텐츠 스왑 자체에는 등장/퇴장 효과를 넣지 않는다. 드래그 중에는 transition 을 끊어 손을 즉시 따른다. (2026-07-22 개정 — 초안의 도착 슬라이드 효과는 폐기)
