...
4. **실이동과 내용 인계**: FLOW 스테이션 변경 시 최종 공간 배치를 먼저 확정하고, 레일과 이동 영향을 받는 pane은 출발점과의 차이를 같은 340ms compositor `translate`로 0까지 보간하는 FLIP 주행을 한다. `left`·`width`의 프레임별 보간은 독립 레이아웃·페인트로 중간 프레임을 찢으므로 금지한다. 레일 프레임은 주행 내내 불투명하다. 서로 다른 투영 내용이 교체될 때는 셸이 아니라 내부 내용만 `A1 → A0 → B0 → B1` 순서로 인계한다: 출발 내용은 여정 38%까지 사라지고, 38–46%는 양쪽 내용이 모두 투명한 짧은 중립 구간이며, 도착 내용은 46%부터 나타나 88%에 완전히 보인다. 같은 instanceKey 및 초기 마운트에는 인계 효과가 없다. 드래그 중에는 애니메이션을 끊어 손을 즉시 따른다.
