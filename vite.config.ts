...
export default defineConfig(async ({ mode }) => {
  // 제품 프레임워크는 자기 이름을 반드시 선언한다. 누락/오타를 Tauri로 해석하는 fallback은 없다.
  // Vitest만 제품 프레임워크가 아닌 중립 어댑터를 쓴다. 프레임워크 테스트는 각 구현을 직접 import한다.
  // @ts-expect-error process is a nodejs global
  const declared = process.env.SOKSAK_FRAMEWORK;
  const framework = mode === "test" ? "neutral" : declared;
  if (framework !== "tauri" && framework !== "electron" && framework !== "neutral") {
    throw new Error(
