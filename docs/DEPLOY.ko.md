...
### 4a. 소스 워크스페이스 vs 런타임 홈 — 개발자 트리

런타임 홈(`~/.soksak*`)은 설치된 아티팩트를 담는다 — repo 를 개발하는 곳이 아니다. 소스는
역할별로 한 곳에만 두어, 같은 repo 의 두 체크아웃이 어긋나지 않게 한다:

- **`~/soksak/<repo>`** — **홈-로드가 아닌** repo 의 소스 워크스페이스: 코어 모노repo
  (`~/soksak/core`, 유일 정본 체크아웃), spec, contract, 사이드카 소스, 그리고 프로젝트가
  수정하는 서드파티 포크(`~/soksak/vt100-rust`, `~/soksak/wezterm`, …). repo 당 한 체크아웃 —
  추가 작업은 두 번째 clone 이 아니라 `git worktree` 로. 활발히 개발 중이 아닌 repo 는 여기
  두지 않는다 — 원격이 정본이고 필요 시 재클론한다.
- **`~/.soksak-dev/plugins/<id>`** — **플러그인**의 개발 소스 정본. 플러그인은 홈-로드 유닛이라
  편집 소스가 `~/soksak/` 이 아니라 dev 홈에 산다.
