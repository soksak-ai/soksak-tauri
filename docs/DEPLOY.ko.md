...
소스는 역할별로 한 곳에만 두어, 같은 repo 의 두 체크아웃이 어긋나지 않게 한다. 홈에 존재하는
유닛인지에 따라 두 역할로 나뉜다:

- **dev 홈은 홈-존재 유닛의 작업 공간이기도 하다.** `~/.soksak-dev` 안에서 유닛 자기 폴더 —
  `plugins/<id>`·`sidecars/<id>`·`kits/<id>` — 는 in-place 편집하는 git 체크아웃이다: 같은
  폴더가 **설치된 유닛이자 편집 소스**(편집 → reload / rebuild). debug·release 홈은 같은
  폴더를 내려받은 아티팩트로만 담고, 편집하지 않는다.
- **`~/soksak/` 은 홈에 없는 repo 의 작업 공간이다.** `~/soksak/core` = 코어 모노repo(유일
  정본 체크아웃). `~/soksak/<name>` = 그 밖의 파생 개발: 프로젝트가 수정하는 서드파티 포크
  (`~/soksak/vt100-rust`, `~/soksak/wezterm`, …)와, 활발히 작업 중인 비-홈 유닛(spec, contract)
  소스. repo 당 한 체크아웃 — 추가 작업은 두 번째 clone 이 아니라 `git worktree` 로. 활발히
  개발 중이 아닌 repo 는 여기 두지 않는다 — 원격이 정본이고 필요 시 재클론한다.
- **제거된 소스는 삭제가 아니라 아카이브** — 중복·폐기 체크아웃은 fsck 무결성 + 브랜치 수
  대조 게이트를 거쳐 백업 트리로 `mv`(모든 브랜치 보존)한다. 백업 트리는 정리 전 스냅샷과
  repo 별 `git bundle --all` 미러도 담아, 어떤 브랜치·커밋도 유실되지 않는다.
