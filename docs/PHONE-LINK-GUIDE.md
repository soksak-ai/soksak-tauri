...
- **P4 — Cloudflare Worker convergence**: the Worker is **built and locally verified** (`worker/`, 69 tests — the Paddle license webhook + the Ed25519 challenge-response `/verify`, device pairing, and revoke; see `worker/README.md`), and the **convergence foundation** is in place — a cross-language golden vector proves a Worker-issued capability assertion verifies byte-for-byte on the Rust `remote::auth` side (`docs/license-system-design.md` §4.3; an additive Rust test, no floor change). Remaining: **deployment** (needs the user's Cloudflare/Paddle accounts) and **switching the live issuer** from device-self-signed to Worker-issued — a deliberate model change that re-wires the verified client (not yet taken; the live model stays peer-signed).
