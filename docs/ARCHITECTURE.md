...
### A17. One environment, one home — the framework names, it does not divide.
An identifier is `com.soksak.<framework>.<env>`, and its two axes decide different things. **The home comes from `env` alone** (`*.app` → `~/.soksak`, `*.dev` → `~/.soksak-dev`, `*.debug` → `~/.soksak-debug`); the framework does not divide it. What lives in a home — plugins, projects, themes, the vault — belongs to the user, not to the framework that renders it, and a home split per framework empties it the moment the framework changes. That emptiness surfaces as "my data is gone", never as an error. The **framework** axis divides exactly **two** things: the product display name (two apps must be distinguishable in the Dock) and the webview profile (`framework-<framework>/`). The profile split is not our choice but the engine's — a shared Chromium profile's ProcessSingleton forwarded the second app's engine launch to the first, and WKWebView already lives apart under the OS path (`~/Library/WebKit/<bundle-id>`). **Nothing else divides by framework.** What a home has only one of — cored, the control socket, the store — is named by the home (`home_identity_of` → `com.soksak.<env>`); the control socket seat is the home's too, so whichever process binds first holds it and the other lives without one (commands from outside arrive through cored's delivery to the window host). Name it after whichever framework launched it and that name is stamped onto a shared resource, and every process that attaches has to work around "is that somebody else's app?" (measured 2026-08-01: cored was running as `com.soksak.tauri.dev` and the CLI carried exactly that workaround — needing the workaround was the evidence of the defect). The measured hazard that motivated this rule is preserved by exactly that split: a shared Chromium profile's ProcessSingleton forwarded the second app's engine launch to the first, so the webview profile must never be a name two frameworks can both claim. Each home owns a complete, independent set: data DB, plugins, sidecars, themes, projects, secrets vault, backups, and the socket all derive from that one root (`home.rs soksak_home()` — the single truth; a new identity gets its own home automatically from the identifier's last segment). Sharing any of it across *environments* lets state cross that boundary. The home is a fixed derivation from the bundle identifier with no runtime env override — not even under a debug or test name (a distribution invariant, enforced by `distribution-invariants-scan`, that stops one identity from pointing at another's home). A black-box e2e isolates *within* the real identity home instead of relocating it: `SOKSAK_DATA_DIR` puts the SQLite DB and `SOKSAK_VAULT_PATH` the vault in a disposable directory (both `#[cfg(debug_assertions)]`-gated), while the home's installed plugins and sidecars stay in place. To reach such an app from the CLI, set `SOKSAK_SOCKET` to its explicit socket path. Also in **debug builds only**, `SOKSAK_E2E_KEK` supplies a deterministic vault KEK (SHA-256 of its value) so black-box e2e opens the vault without a keychain; it is `#[cfg(debug_assertions)]`-gated and compiled out of release, so no env key material ever ships (the release env-unlock `SOKSAK_VAULT_KEY` was removed). The `sok` CLI is a standalone busybox binary and implements the same contract (`cli/src/main.rs home_for_env`); this section is that contract's canonical statement. The release home holds **installed artifacts only** — registry-installed plugins (semver-self-described) and hash-pinned sidecar dists fetched from GitHub releases; no development source ever enters it. The dev surfaces are identity-gated off in release: `plugin.dev.*` refuses, and folders self-described `version=dev|local` are rejected at load. There is no ambient env source injection in any identity — the designated home directories (plugins, sidecars/dist) are the only standing resolution paths; a one-off external plugin load goes through the explicit `plugin.dev.load` command, and a fresh sidecar build is staged into the identity home via `stage.sh`.

### A18. Registries live outside the module boundary — registration cannot vanish.
Registration (commands, views, hooks, bus topics, header/status items) is a **side effect of module evaluation**. When the filling site and the holding site live in different modules, a dev swap can replace **one of them**: the holder becomes a fresh empty one while the filler still believes it already ran, so the registry stays **empty forever**. Measured 2026-07-31: the entire core command catalog disappeared. The window was alive and plugin commands still answered, yet every `ui.*`, `state.*` and `window.*` failed — and to the user it looked only like "the tab's `+` won't create anything" (the `+` button hides itself when zero programs are registered, so the defect disguised itself as a missing button). What worked must not stop working by accident. Therefore **module top-level mutable state is declared through `moduleState("<module path>#<name>", () => …)`** — stored on `globalThis`, the one place a swap cannot reach, and inert in production bundles. Do not split the rule into "registry vs cache": that judgement must be made by hand every time, and a hand-made judgement misses one. One rule leaves no gap, and `module-state-scan` ratchets new arrivals out. The holder and the "already filled" memory must live **together** — if only one survives, the registry either empties out or dies on duplicate registration. And **an empty registry is a different fact from an absent name**: zero has two faces, "looked and found nothing" and "nowhere to look", and the second is a defect, so it answers as `REGISTRY_EMPTY`. Collapsing both into one code leaves no way to read the cause from outside.

### A19. When something is degraded, every response says so.
Observation wiring can die while commands still answer normally. Telling the difference required querying the ledger twice and comparing timestamps — that is manual measurement, not diagnosis (measured 2026-07-31). So degraded axes ride along in the envelope's `degraded` field, whatever was asked; when the axes are healthy the key is absent entirely. Details come from `state.health`: a summary alone cannot count which axis has been failing since when, and what cannot be counted cannot be proven fixed. **A verdict needs "since when should it be there"** — an absence before boot declares its wiring complete (`markRuntimeReady`) is not a defect, and without that signal a booting app and a broken one look identical. **No silent failure**: swallowing and not counting are different things; keep the live path unblocked but count attempts, successes, failures, consecutive failures and the last reason. Zero attempts is *unverified*, not healthy. Count the health of the instrumentation itself — when it dies, that death is not instrumented either.

### A20. A ledger has one owner — the stamp belongs to the window, the write to the windowless process.
The activity `seq` is the cursor basis for the ring and subscribers, so the process holding that ring assigns it. The **write** belongs to cored: only the app process's SQLite returns `SQLITE_NOMEM` for it (measured — same write code, 44 failures in the app, 0 in cored). cored's `activity_persist` therefore persists without stamping and never advances its own cursor; advancing it would create a second owner. **Waiting for a reply deadlocks**: the connection is bidirectional, so a window waiting on cored while cored waits on that window locks both (measured: the whole UI stopped responding). Work that needs no answer is told, not asked. **The evidence of persistence is the stamp** — a resolved call does not mean the entry landed; the reply's `seq` does. The stamp carries the ledger's *name* too: two ledgers each increasing monotonically look healthy from either side. **The signal of damage is time going backwards, not gaps** (`activity_audit`) — two retention tiers leave gaps legitimately, while overwriting pushes a later `ts` into a lower `seq`.

### A21. User actions go through commands — UI and CLI/AI share one path.
The command layer carries observation, normalisation, the envelope, the danger gate and hints. UI calling the store directly skips all of it and becomes a *different path* from the identically-named CLI call — and two paths stay quiet until they diverge. Pairings are not chosen by hand: a store function called by a catalog handler is itself the evidence that the action has a command (`ui-through-commands-scan`). **Raise the UI to the command's axis rather than lowering the axis**: seams are named by the leaf they touch, never by an interior split id (§4). **Expression is not action** — per-frame feedback during a drag is expression; only the landing is recorded as a command.

### A22. One process owns the store — everyone else reaches it through the interface.
When two processes each open the same home's store, SQLite does not refuse; it merely serialises — and that contention *is* the coupling. Ownership is first-come (`store_lock`), and failing to take it is not an error but **delegation** (`StoreRoute::AskOwner`). Boot order is therefore part of the contract: **stand cored up before the ownership decision.** If the app claims first, cored writes without ownership and hits `database is locked` continuously (measured 2026-08-01: 44 times). If cored cannot start, the app claims — either way there is exactly one owner. **Within a process there is one writer connection too**: WAL is concurrent-read, single-write, so opening per publish makes a process fight itself (measured: 27 failures even after the app released the store). Rules take `&Connection` and never own it. **The same name answers the same thing** — a delegating process deserialises the owner's reply into its own command's type, so a shape mismatch breaks *only at the moment delegation begins* (`delegated-shape-scan`). **Delegation does not exempt you from resuming.** A process that released the store has no connection, so its resume query never runs — and a ring starting at 0 makes its stamps **overwrite the owner's past** (measured 2026-08-01: regressions went 2→4 and the first regression seq dropped to 85, because ownership moved without the resume path moving with it). When delegating, ask the owner for the last seq; if you cannot ask, **do not resume — count it.** Not stamping when you do not know beats overwriting. **Do not write "one" and then build two**: a writer singleton in both the ledger path and the command path reproduced the same defect twice (that was the residual 7 failures). There is one place — the store layer.

**Exposure is not an excuse to keep coupling**: making contention visible in `degraded` and moving on leaves the defect alive.

**Notification belongs to whoever owns the store.** Move ownership without moving notification and writes converge on one place while the fact that *something changed* stays inside the writing process — another framework on the same home keeps believing the stale value it holds and **overwrites** the change on its next save. That loss does not look like an error (measured 2026-08-01: `app.emit("data-change")` reached only that framework's windows, and the other framework had no such emit at all). A process with no windows hands the fact to the ones that have windows (`control::broadcast`), and the payload keeps the **same shape** the framework used to emit — subscribers do not know the origin, and a different shape makes identical code see different things per framework. A write performed by the app still passes through this hub (`data_change_notify`), and never both paths at once: the broadcast comes back to the writer too, so also emitting locally delivers the same event twice.

**Do not count a rule — require it.** Kept as "write commands notify" and enforced by scanning source, a planted violation passed because one function boundary was parsed wrong. So the **type requires it** instead: the write door (`with_write`) cannot be passed without producing what changed (`Changed`), and omitting it fails to compile. Changes that cannot be pinpointed (import, restore) say *whole*; changes that touched nothing say `none()` — without a way to say that, callers build the notification conditionally and route around the enforcement. The gate is the layer after: it counts writes that never enter that door. And it cannot count at all when the same decision has two shapes — one command spelled its ownership check by hand and was silently missing from the tally.
...
### A29. Pin fixes both position and identity.

Changing focus under a `pin` sidebar changes selection state; it is not a layout operation. The sidebar station and DOM identity, plus the split tree order and rects, remain unchanged. FLOW policies, including `railPullFocused`, cannot override this invariant.

The relation follows actual geometry. A selected pane touching the fixed sidebar on its left receives a left union border; one touching on its right receives a right union border. If neither edge touches, the intervening space is not promoted into a relation surface: the sidebar and selected pane each receive an independent active border. Borders are therefore results of fixed geometry and selection, never causes of rearrangement.
...
- `app.webview.captureFull(label, path, width, height)` — a full-document PNG of the explicit
  ...
  event-confirmed viewport set. It hides only the default repeated scrollbar during the transaction and
  must restore the exact inline overflow declarations and original scroll position before succeeding.
