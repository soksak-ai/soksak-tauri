...
(xterm.js) and `soksak-plugin-terminal-ghostty` both implement `soksak-spec-plugin-terminal@1`; the core
...
(`soksak-spec-sidecar-browser@1`) the skeleton relays without understanding.
...
- **L2 — contract-pin: the target.** The manifest declares `implements: ["soksak-spec-<kind>-<domain>@<major>"]`; discovery is contract-addressed and implementation-blind; conformance proves declared ≡ actual.
...
The `soksak-spec-<kind>-<domain>@<major>` grammar, version negotiation, and conformance follow the sidecar contract law ([SIDECARS.md](SIDECARS.md)), extended to the plugin layer. That extension amends NAMING.md §8 — never as a silent violation, only as an explicit re-legislation commit (the C5 procedure).
