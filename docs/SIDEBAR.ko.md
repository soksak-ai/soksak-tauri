...
- **투영 슬롯** — 스페이스가 `railBindingViewId`로 고정한 단 하나의 콘텐츠 뷰가 선언한 사이드바가 해소·렌더되는 곳. 패널·그룹·활성 탭 포커스는 레일 위치만 바꾸고 투영 콘텐츠는 바꾸지 않는다. 레일 상호작용은 둘 다 바꾸지 않는다.
...
- **안정성**: 한 스페이스에는 결부 하나만 있다. 패널 포커스가 바뀌면 FLOW 위치만 이동하고 슬롯·인스턴스·스크롤·상태는 유지된다.
...
- **복원**: 콜드 재기동은 투영을 동형으로 재현한다 — 스페이스별 결부·슬롯 구성·instanceKey 연결·핀·구조 상태. 핀과 자동 핀 기억(`seen`)은 프로젝트별 창 스냅샷에 영속된다.
...
| `projection.changed` | 해소 지문 — 스페이스 결부·슬롯 상태·핀 — 이 바뀔 때 발화한다. 포커스만 바뀐 경우에는 발화하지 않는다. 부트 관측은 무발화다. |
