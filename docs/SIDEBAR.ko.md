...
- **투영 슬롯** — 스페이스의 단일 결부 뷰(`railBindingViewId`)가 선언한 사이드바가 해소·렌더되는 곳. 뷰 활성화는 **해소 결과가 달라질 때만** 스페이스를 재결부한다 — 같은 기능 간 이동(터미널↔터미널)은 결부를 유지하고 FLOW 위치만 옮기며, 다른 기능(또는 다른 per-view 문서)으로의 전환은 투영을 교체한다. 레일 상호작용은 둘 다 바꾸지 않는다. 레일의 FLOW 스테이션이 바뀌면 사이드바 프레임이 **실좌표를 주행**한다 — 창(pane) 뒤로 지나가는 실제 이동이며, 콘텐츠 스왑 자체에는 등장/퇴장 효과가 없다. 사이드바는 hide→show 연출이 아니라 실제로 이동한다.
  모든 슬롯은 **공통 양식**으로 렌더된다: 호스트 소유 헤더(기능 아이콘·제목 + 레일 조작 — 좌측 첫 슬롯이 레일 모양 토글을 가진다) 아래 본문만 플러그인의 것이다. 같은 참조를 공유하는 기능들은 양식과 인스턴스를 공유한다. 레일 모양은 2종이며 `railLook` 설정으로 영속된다: `pane`(카드 틴트+elevation, 분할창처럼) | `ground`(바닥에 눕는 평면, 경계선만).
