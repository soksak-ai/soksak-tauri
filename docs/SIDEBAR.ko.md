...
- **결부 통보**: 인스턴스가 유지돼도 결부가 바뀌면 뷰가 안다. `shared` 는 결부가 오가도 같은 instanceKey 라 remount 가 없으므로, 통보가 없으면 화면에 **직전 결부의 내용**이 남는다(실측 2026-07-31). 통보 대상은 ctx 의 binding 축 전부이고 축의 단일 진실은 `src/plugins/viewContext.ts` 다 — 필드 이름을 문서에 나열하지 않는다. `update` 를 구현한 provider 는 새 ctx 를 받고 인스턴스를 지키며, 미구현 provider 는 remount 된다(남의 결부 데이터를 그리는 것보다 구조 상태를 잃는 편이 옳다).
...
| `displayLayout` / `switched` | pin: 정본 분할 트리의 순서·rect·객체 정체성을 유지하고 `switched=false`다. 포커스 변경은 배치 조작이 아니다. flow: 포커스 왼쪽 선이 막혔고 재배치 정책이 켜졌을 때만 바로 왼쪽 leaf 형제와 최소 스위칭한다. `switched`는 FLOW에서 만들어진 인접에 점선 봉합을 붙이는 유일한 근거다. |
...
pin의 관계 표시는 판을 옮기지 않고 실제 rect만 판정한다. 선택 판의 오른쪽 변이 레일 station에
닿으면 왼쪽 합성 보더, 선택 판의 왼쪽 변이 닿으면 오른쪽 합성 보더를 그린다. 어느 변도 닿지
않으면 사이 공간을 연결하지 않고 레일과 선택 판에 독립 활성 보더 두 개를 그린다. 관계 노드는
`data-placement`, `data-connected`, `data-side`, `data-rail`, `data-box`로 이 판정을 노출한다.

탭 최대화는 임시 투영이며 저장된 PIN station이나 분할 트리를 쓰지 않는다. 선택 판이 원래
PIN 왼쪽이면 판이 왼쪽을 채우고 레일은 오른쪽 경계에 서며, 선택 판이 PIN과 같거나 오른쪽이면
레일이 왼쪽 경계에 서고 판이 오른쪽을 채운다. 복원하면 저장 station과 정본 분할 rect로 정확히
돌아간다.
...
복도 identity는 `useArrangementPhase`와 projection-frame 단위 테스트가 시행한다.
`scripts/e2e/slot-freeze.mjs`는 별도로 실제 패널 탭을 교차 클릭하며 전 프레임을 기록해, 레일 결부가
이동하는 동안 문서 밖 콘텐츠 표면이 계속 살아 있음을 증명한다.
