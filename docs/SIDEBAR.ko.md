...
- **투영 슬롯** — 스페이스의 단일 결부 뷰(`railBindingViewId`)가 선언한 사이드바가 해소·렌더되는 곳. 뷰 활성화는 **해소 결과가 달라질 때만** 스페이스를 재결부한다 — 같은 기능 간 이동(터미널↔터미널)은 결부를 유지하고 FLOW 위치만 옮기며, 다른 기능(또는 다른 per-view 문서)으로의 전환은 투영을 교체한다. 레일 상호작용은 둘 다 바꾸지 않는다. 재결부는 미끄러져 들어오는 전환과 함께 도착한다 — 사이드바는 새로 생기는 게 아니라 옮겨온다.
