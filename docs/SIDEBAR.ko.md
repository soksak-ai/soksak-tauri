...
- **투영 슬롯** — 결부된 콘텐츠 뷰가 선언한 사이드바가 해소·렌더되는 곳. 결부는 세션 활성 체인의 말단(활성 프로젝트 → 활성 스페이스 → 활성 그룹 → **활성 뷰**)이며, 그룹 내 활성 탭 전환도 결부 변경이다. 레일 상호작용은 결부를 움직이지 않는다.
- **핀 스택** — 사용자가 고정한 rail 뷰들. 콘텐츠와 같은 분할/탭 머신으로 배열된다. 핀은 결부 변화에 흔들리지 않는다.
- **레일 푸터** — 하단에 상주하는 `rail-footer` 뷰.

도구는 사이드바를 자기 뷰 안에 그리지 않는다. 콘텐츠 뷰는 사이드바를 *선언*하고, 레일이 그것을 투영한다. 선언에서 좌는 필수, 우는 선택이다. 이행기 동안 미선언 레거시 도구는 투영 밴드를 조용히 접는다. 요란한 A1 파서 게이트는 함대 선언 완료 후에만 활성화된다(결정 기록 §7).
...
- 참조되는 뷰는 `rail` placement를 가진다. `rail-footer`는 하단 상주 슬롯이다. 구 이름(`sidebar-left`·`sidebar-right`·`sidebar-footer`)은 한 버전 동안 앨리어스로 수용된다.
...
- **안정성**: 결부가 이동해도 슬롯이 같은 instanceKey로 해소되면 그 슬롯은 전환되지 않는다 — 인스턴스·스크롤·상태 유지. 터미널과 에디터가 같은 파일트리 계약을 참조할 때 레일이 미동도 않는 이유다.
- **keep-alive**: 투영 인스턴스는 마운트 유지·display 토글. 죽은 per-view 인스턴스(결부 뷰 닫힘)와 흡수된 인스턴스는 퇴거된다.
- **흡수**: 핀된 shared 참조는 자기 투영 슬롯을 흡수한다(`satisfied-by-pin`) — 핀 스택이 단일 렌더를 소유한다.
...
- **복원**: 콜드 재기동은 투영을 동형으로 재현한다 — 결부 체인·슬롯 구성·instanceKey 연결·핀·구조 상태. 핀과 자동 핀 기억(`seen`)은 프로젝트별 창 스냅샷에 영속되며, 투영 이전 스냅샷은 옛 사이드바 배치를 핀으로 채용하는 1회 마이그레이션을 거친다.
...
| `projection.changed` | 해소 지문 — 결부·슬롯 상태·핀 — 이 바뀔 때 발화한다. 그룹 내 탭 전환·슬롯 승격/강등 포함. 부트 관측은 무발화. 이 이벤트가 유일한 push 채널이다 — 레일도 외부 소비자도 폴링하지 않는다. |

`plugin.view.open`은 `rail` placement를 좌 레일 핀 추가로 라우팅하고(레일 열림), `rail-footer`는 열기 대상이 아니라 거부한다. dev 소스 적재는 dev identity 전용이며, debug·release 홈은 발행본 설치로 검증한다.
...
- **흡수**: 투영 중인 shared ref를 핀 → `satisfied-by-pin` 단일 렌더; 해제 → `live`.
