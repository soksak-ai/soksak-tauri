...
4. **검사는 코드 곁에 두지 코드 안에 두지 않는다.** 모듈과 그 검사는 다른 파일이다. 인라인
   `#[cfg(test)] mod tests` 는 한 파일에 독자를 둘 앉힌다 — 규칙과 규칙의 증명이 함께 자라
   어느 쪽도 따로 읽을 수 없게 되고, 그 파일의 길이가 뜻하는 바가 달라진다. Rust 는 형제 파일과
   `#[cfg(test)] #[path = "x_tests.rs"] mod tests;` 로 한다.
5. **커진 파일은 역할을 둘 이상 지고 있다 — 역할로 나눈다.** 크기순도 알파벳순도 아니다:
   **무엇의 규칙인가**로 나눈다. 크기는 들여다보게 만드는 증상이고, 가르는 선은 언제나 역할이다.
...
인라인이다 — 두 방식이 섞여 있고, **형제 파일이 기준이며**(규칙 4) 인라인 쪽이 남은 일이다.
...
떨어져 나온 다섯이 나머지가 향할 모양이다 — `x.rs` 가 규칙을, `x_tests.rs` 가 그 증명을 지고,
어느 쪽도 다른 쪽 안에 숨지 않는다.
