...
- **Substrate (single source of truth)** = Command Registry (`src/commands/registry.ts`: `Map<string,CommandSpec>`, `catalogJson()`) + Socket Server (`frameworks/tauri/src/ipc.rs`: Unix domain socket JSON-RPC, multi-window routing, danger gate; wire contract & version negotiation = [SOCKET-PROTOCOL.md](SOCKET-PROTOCOL.md)) + Activity Hub (`frameworks/tauri/src/activity.rs`: the execution stream, P11–P12). The only truth for a command's existence, schema, permission, and execution. Core `register()` calls (~140) and plugin `contributes.commands` converge into one Map.
