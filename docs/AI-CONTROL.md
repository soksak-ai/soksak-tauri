...
- **Teaching (above the channels)** = the Skill (soksak-control). It teaches agents **how to use** the channels — how to discover, not a command list. It is not a transport.
...
**P9 — `sok` binds to exactly one environment; silent cross-env is betrayal.** There are three app identities (`com.soksak.{dev|debug|app}`), so sockets are separate (`~/.soksak/com.soksak.<env>.sock`). `sok` binds to exactly one environment and never silently attaches to another. Socket resolution priority: ① `SOKSAK_SOCKET` (injected into the PTY by the app — authoritative) > ② `--env`/`SOKSAK_ENV` > ③ argv0 suffix (`sok-dev`→dev, `sok-debug`→debug, `sok`→app; busybox pattern). If the resolved environment's socket is missing, that is an **error** — no substitution with another env's socket. "Grab the one that is alive" is abolished.

**P10 — Install is idempotent and scoped by ownership.** Regenerate what is ours wholesale; preserve what is the user's.
- **Fully-owned artifacts** (`soksak-control/SKILL.md` — a dedicated directory) are overwritten entirely (regenerated). Hand edits are not preserved — the "AUTO-GENERATED by `sok skill install`" header states that edits are void (source = `sok commands`).
...
- **Mechanism**: `sok skill install` writes a **trigger skill** (SKILL.md with frontmatter name+description) — `--claude`→`.claude/skills/soksak-control/`, `--codex`/`--gemini`→`.agents/skills/soksak-control/` (shared). The body derives the live domain map via `skill_doc()` (core fallback when the app is down). The description auto-triggers.
...
- **Skill** — the `SKILL.md` frontmatter `description` is the trigger (identical across Claude/Codex/Gemini — confirmed against 2026 official docs). The model auto-invokes on task match. Slash `/soksak-control` is auxiliary.
...
- ✅ Environment binding / betrayal cut (U8): `resolve_socket` — SOKSAK_SOCKET > --env > argv0; silent cross-env abolished (P9). App·CLI paired builds.
