...
영문 정본: [multiplatform-engine-strategy.md](multiplatform-engine-strategy.md) — 영문이 정본이다.
...
첫 실증: **astryx** — macOS WKWebView에서 정상 동작 불가 → Chromium 사이드카 표면으로 승격. 아래 offscreen 호스팅 모드가 그 수단이며, 엔진 E2E 하니스로 단언하고 인앱에서 육안 확인했다.
...
compositing 축 실증(2026-07-08): offscreen 호스팅 모드(공유 텍스처 → 엔진 소유 레이어)가 엔진 프로토콜의 additive 어휘로 구현됨 — **기능에 대한 코어 변경 0**(A9 실증). 픽셀·입력(마우스/휠/키/한글 IME)·cefQuery 브리지는 엔진 E2E 하니스가 단언한다. 검증된 부정 지식(픽셀-over-IPC-into-DOM 불가)은 이 구현으로 대체되었다 — 픽셀은 프로세스 안에서 GPU 핸들로만 이동한다. (별도의 코어 소유 호스트 컨테이너 격리는 SIDECARS.md·webview.rs에 문서화.)
