...
nodes, and a receipt published by the ledger that owns the surface; it never derives one ledger
from another. Slots and renderers are read from public DOM; a native surface is read from the
AppKit PaneSurfaceHost ledger (host frame plus member frame), never copied from the DOM projection
that predicted that frame, and a public topology string is accepted only when it ends at the
natively registered member label. The envelope carries which ledger produced each observation and
when the surface ledger sampled itself, so the judge decides whether all three were read inside one
settled window; coordinates may differ only by physical-pixel rounding.
...
transaction. The mapper never copies a requested `size` into the observed window geometry. Each
step's intent (`phase`) is declared by the harness that builds the sequence, in the same place as the
size, and the observation reports it back; the judge compares that declaration against the requested
size. The window already fills the screen, so `wide`/`tall` mean an aspect the sequence broke by
cutting one axis, not a size larger than the original. Neither an ascending phase order nor
"restore appears only once, last" is required — returning to the original size several times is the
round trip itself.
...
| B10 | Hostile rapid whole-window resize is affine and restores | Every finite resize transaction asserts DOM/native coordinate agreement and restoration of the original final geometry. There are at least four transactions, and every one requests a size different from the one last observed. Each transaction declares its intent (`shrink`, `wide`, `tall`, `restore`), that declaration must hold for the requested size against the baseline, and all four intents must appear at least once. The last transaction is a `restore` whose observed geometry equals the baseline exactly. A transaction that returns to the original size mid-sequence is part of the round trip and is never refused. |
| B11 | Pane resize round trip + wheel `0→480→0` + tab-targeted full capture | Each axis is asserted from its own measurement. Pane: the pane rect, the native surface rect above it, and the document viewport width inside it all move by the requested gutter drag dx and all return (the left pane widens while the right pane narrows by the same amount and shifts its x). Wheel: alongside the `0→480→0` positions, the page's own wheel/scroll event counts increase on every leg and the accumulated delta matches the requested direction. The page publishes its wheel count to the document so the harness can wait for the wheel to reach the page before reading the ledger — a scroll landing and a wheel event reaching the page are separate facts, and every engine delivers the first one earlier, so waiting only on the scroll reads a count that has not been taken yet. Capture: the explicit view, path, and byte counts are exact, and the artifact PNG's IHDR size covers the whole document at one uniform scale on both axes — comparing the receipt's requested document size against the harness's own read of it is circular and is not accepted. The four measurements carry distinct layout settle epochs, and the gate is stamped once, after the pane resize has been measured. |
| B12 | macOS traffic-light center/composition agreement under cold start, hostile resize, and titlebar-height changes | A window may be presented only after restored logical size and saved zoom have received their native applied ACK, then the post-zoom public titlebar has composed GREEN. Tauri uses one AppKit main-thread transaction and one paint owner: the three derived backing regions, three live AppKit buttons, and three DOM reservations assert mapping, containment, vertical centers, and resize agreement. The startup gate flushes the AppKit display transaction and then takes a read-only actual-versus-declared geometry sample; an earlier `lastApplyOk` receipt cannot authorize presentation if the displayed button frames or owner hierarchy no longer hold. `trafficLightPosition` is forbidden in Tauri configuration while this dynamic composer is installed because it installs Tao's competing fixed-y draw owner and also feeds non-child Wry paths; the adapter preserves the initial AppKit horizontal spacing and derives vertical position only from the public DOM titlebar. Public async resize owns its `Window` through an awaited oneshot ACK; timeout-then-late mutation and queued bare `NSWindow` pointers are forbidden. `titlebar.height.set {height}` changes the public DOM geometry, waits for a complete paint boundary, and returns the same strict native receipt; `titlebar.height.reset` restores the exact prior inline height/flex basis. Every cold-start and height sample must have zero button/backing center delta (rounding-only tolerance). Electron declares its own titlebar composition axes (`titlebarComposition` — buttonPositions, backingPlane, paintOwner), and the judge measures whether that declaration actually answers live. When a declaration is missing or does not answer, the cell is `blocked` with that fact named; DOM clones or synthesized native frames are forbidden substitutes. Non-macOS reports mark this gate statically `not-applicable`. |
...
browser surface, so a framework without a native child surface (`nativeChildWebview: false`) does
not hold that fact at all — painting it red makes the standard unreachable, counting it green passes
a cell nobody measured. Do not split on the framework name: every new name adds a branch and finds
no place of its own. The
