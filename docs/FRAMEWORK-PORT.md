...
- **Nothing is left holding the socket.** Past the timeout the framework reaps the process it started before reporting, so the reason is the real one and no orphan keeps the path. cored has the matching half: if something already serves that path it withdraws rather than unlinking a live socket. “Served” means a bounded `system.hello` round trip whose reply identifies `role:cored`; a successful Unix `connect` is insufficient because a child can inherit the listener FD, accept connections, and answer no cored protocol. The spawner and cored's singleton gate use the same probe.

**Tauri does the same** (`frameworks/tauri/src/cored_host.rs`). It probes the seat, adopts a live cored or spawns one with the identity it already holds, and then registers its windows over that connection with `control_host_attach` — cored has no window, so a command that arrives from outside reaches a screen only because a window owner made itself the delivery route. An unregistered window host cannot receive commands, and cored refuses it as `NO_HOST`.

- **Adoption precedes binary resolution.** When `<home>/cored.sock` is actually served, Tauri adopts that connection without requiring a bundled sidecar path. A spawn-preparation defect must not sever an already-live shared store connection. Only an empty seat resolves `SOKSAK_CORED_BIN` or the `soksak-cored` sibling of the app executable.
- **App bundles carry cored.** `make build-dev`, `make build-debug`, and `make build` give the app and cored the same Cargo profile and target triple, then include `Contents/MacOS/soksak-cored` through Tauri `externalBin`. The `<name>-<target-triple>` regular-file copy required by Tauri is an untracked bundle input; the final app contains one executable. HMR `make dev` does not bundle, so it builds cored without this staging step.
...
- **A created window is not complete until it is addressable.** `window_create` reports the new live-window facts and waits for the correlated `control_windows` acknowledgement before returning success. This is event completion, not polling. A restored window created with `focus=false` emits no focus event, so focus can never be the registration trigger. If registration fails, the just-created window is destroyed instead of leaving `window.list` and cored's address book with different truths.
...
- **Nothing comes back through the relay.** The window executor calls `invoke("cmd_result", {id, result})`, and that call rides the ordinary bridge. A side effect that can destroy that bridge, such as process termination, is registered as an executor `afterReply` transaction and runs only after `cmd_result` delivery succeeds. A failed delivery does not commit the side effect, preserving a caller retry. Building a return path in the relay would give one command two ways to answer.
...
A finite command that can outlive the default response cap declares `timeoutMs` in the public socket
envelope at the call site. It is never smuggled into command params or replaced with a fixed sleep,
retry loop, or an asynchronous success that hides incomplete work.
