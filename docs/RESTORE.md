...
- **H-ASK — never make a peer do what it cannot do.** The handoff plan is drawn by the *departing* daemon, so staging a new binary does not make the running one safe. The app asks first: the daemon declares `handoffContract` on `Ping` (absent = predates the contract) and `pty.daemon.upgrade` refuses outright, naming the one remedy — restart it once. The level is readable from `pty.daemon.status` (`handoffContract` / `handoffContractRequired`), so upgradability is known before trying, not discovered by losing a shell. The reply is also checked afterwards: if the same pid still answers, nothing was handed over and the command says so instead of reporting success.

Every crossing is on the activity feed: `pty.stream.ended`, `pty.stream.reattached`, `pty.session.gone`, `pty.stream.reattach.failed`, `pty.daemon.upgrade.refused`. A frozen pane must always be readable from the ledger, never inferred from source. `scripts/e2e/pty-handoff.mjs` is the gate — it sweeps every terminal engine across repeated upgrades and asserts the same shell keeps answering with no reload.
