...
- **귀의 문장 규칙**: 낭독 문장에는 경로·식별자(창 label·해시·URL)를 싣지 않는다 — 그것은 눈의 정보(`message`)다. 그런 `message`를 가진 명령은 봉투 `tts`(문자열)로 귀용 문장을 따로 선언한다(예: `window.snapshot` — message 는 저장 경로, tts 는 "화면을 저장했어요"). message(표시)와 tts(낭독)는 별도 축이며, 새 축을 만들지 않는다.
