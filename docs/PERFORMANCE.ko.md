...
부팅은 띄운 시각부터 앱이 첫 명령에 답할 때까지로 잰다. `make boot-latency` 가 냉시동으로
돌린다. 띄운 쪽이 `open` 을 부른 시각을 기록하고, 모든 구간이 활동 원장으로 답해 프로세스
쪽과 프론트가 한 축에서 읽힌다.

시작 시각은 띄운 쪽이 소유한다. 부르는 쪽에서 재면 `make` 와 `node` 기동이 앱의 부팅으로
둔갑한다 — 그렇게 재면 1658ms, 띄운 시각부터 재면 717ms 였다.

계측은 **한 번만 묻는다.** 되물으면 잃어버린 배달이 다음 물음에 가려진다. 창이 리스너를 달기
전에 나간 `cmd-request` 는 사라지고, 보낸 쪽은 `DEFAULT_REPLY_WAIT_MS` 를 채운다. 그 한 건이
11.7 초 부팅 중 9.4 초였고, 창이 밀린 배달을 회수하면서(`cmd_listener_ready`) 사라졌다.

구간(2026-08-08 실측, 웜 시동 총 ~500ms):
...
- 저장소 부팅 개방이 `PRAGMA quick_check` 로 전수 검사를 돈다. 106MB 실측 1471ms 로 개방의
  99.8%(`init_base` 0ms, `reconcile_fts` 2ms, `write_canary` 0ms). `data.stats` 가
  `openTimings` 로 답한다. 게이트는 그대로 둔다 — cored 가 저장소를 소유하고 앱보다 오래
  산다. 다만 저장소가 자라면 그 한 번이 자란다.
- cored 가 없을 때 세우는 비용: 285~615ms.

`controller.activate` 는 코어가 기다린다. 그래서 플러그인이 거기서 하는 일이 그대로 부팅에
실린다. 각 활성화는 이름과 소요로 도장이 남는다(`plugin-activate:<id>`, 실패해도 남는다).
활성화는 등록이다 — 문서나 상태 복원은 그것을 필요로 하는 뷰의 일이지 활성화의 일이 아니다.
...
## 플러그인 번들은 프로세스 경계를 안 지난다

부팅이 기다리는 것은 **옮기는 양**이지 묻는 횟수가 아니다. 실측 2026-08-08: 번들 34 개를 하나씩
읽으면 벽시계 935ms, 한 호출로 묶어도 818ms 였다. 아무것도 나아지지 않았는데, 어느 쪽이든
23.8MB 중 약 15MB 가 JSON 문자열로 직렬화돼 프로세스 경계를 건넜기 때문이다.

엔진은 같은 파일을 자기 자원 경로로 읽을 수 있고, 그 길에는 직렬화가 없다. 프레임워크 계약이
`assetUrl(path)` 로 답한다 — 파일을 자기 엔진의 이름으로 부르는 법은 각 프레임워크가 알고,
코어는 그 주소를 `fetch` 한다. 번들을 나르던 IPC 명령은 지웠다. 두 번째 통로를 남겨 두지 않는다.

고친 뒤 실측: **번들 34/34 를 40ms** — 같은 파일, 같은 부팅.

매니페스트(`plugin.json`)는 그대로 IPC 를 지난다. 수 KB 짜리이고, 그 읽기는 경로 판정
(`expand` + `project_root_verdict`)을 지난다 — dev 적재가 사용자가 준 임의 경로를 받으므로 그
판정이 곧 경계다. 크기가 값이 아닌 자리에서 판정을 버릴 이유가 없다.

`make boot-latency` 가 이것을 기계로 판정한다. 도장 하나를 읽고
(`plugins:prefetched:<받은>/<원한>:<ms>`) 하나라도 못 왔거나 예산을 넘으면 red 다. 통로가 막혀도
앱은 안 죽는다 — 활성화가 하나씩 다시 읽어서 느려진 채로 정상처럼 돈다. 그 회귀를 보이게 하는
것이 이 도장을 세는 이유다.
