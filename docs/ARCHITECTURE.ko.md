...
- **L2 — 계약-핀: 목표.** 매니페스트가 `implements: ["soksak-spec-<kind>-<domain>@<major>"]` 를 선언한다; 발견은 계약 지정·구현체 무차별이다; conformance 가 선언 ≡ 실제를 증명한다.
- **L3 — 이벤트/데이터: 선언된 스키마로만.** `contributes.events` 는 장식이 아니라 검증 대상이다. 미선언 형태를 절대 발행·소비하지 마라.

### C4. 계약 판올림은 명시 재입법이다.
`soksak-spec-<kind>-<domain>@<major>` 문법·판 협상·conformance 는 사이드카 계약법([SIDECARS.md](SIDECARS.md))을 준용해 플러그인 계층으로 확장한다. 그 확장은 NAMING.md §8 의 개정이다 — 무언 위반으로는 절대 하지 않는다. 명시 재입법 커밋(C5 절차)으로만 한다.
