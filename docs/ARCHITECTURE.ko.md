...
- **L2 — 계약-핀: 목표.** 매니페스트가 `implements: ["<scope>-spec@<major>"]` 를 선언한다; 발견은 계약 지정·구현체 무차별이다; conformance 가 선언 ≡ 실제를 증명한다.
- **L3 — 이벤트/데이터: 선언된 스키마로만.** `contributes.events` 는 장식이 아니라 검증 대상이다. 미선언 형태를 절대 발행·소비하지 마라.

### C4. 계약 판올림은 명시 재입법이다.
`<scope>-spec@<major>` 문법·판 협상·conformance 는 사이드카 계약법([SIDECARS.md](SIDECARS.md))을 준용해 플러그인 계층으로 확장한다. 그 확장은 NAMING.md §8 의 개정이다 — 무언 위반으로는 절대 하지 않는다. 명시 재입법 커밋(C5 절차)으로만 한다.

### C5. 기준은 무언으로 약해지지 않는다.
> 사용자 원문 그대로: "기준을 만들고 테스트를 하다가 그 기준을 달성하지 못하는 상황에서 기준을 무너뜨리는 건 배신/배반 행위다. 절대 해서는 안 된다. 단, 기준 자체가 잘못이면 반드시 문제 제기하여 정정해야 한다."

구현이 기준을 못 맞춘다고 기준을 절대 낮추지 마라. 올바른 기준에 테스트가 빨갛다면 고치는 것은 구현·fixture·문서다 — 기준이 아니다. 기준 자체가 틀렸다면 명시적으로 문제를 제기하라: 기준은 명시적 문제 제기와 그 뒤의 재입법 커밋으로만 바뀐다. 무언 완화 금지. 무언 예외 금지. 이 법 아래의 모든 게이트 신설·기준 변경 — C1 allowlist 포함 — 은 이 절차를 지난다.
