...
// tauri.conf의 trafficLightPosition은 Tao에 고정-y draw callback을 설치하고, non-child
// Wry 경로에도 같은 고정 목표를 전달한다. DOM 높이 변경 뒤 이 거래와 경쟁하므로 함께 선언하지 않는다.
//
...
mod tests {
    ...
    fn dynamic_dom_composition_has_no_competing_tao_or_wry_traffic_light_owner() {
        ...
        assert!(
            ...
            "trafficLightPosition installs Tao's fixed-y draw owner and also feeds non-child Wry paths; the DOM-driven AppKit composer must be the only position owner",
        );
    }
