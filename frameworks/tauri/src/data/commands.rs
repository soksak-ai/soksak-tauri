...
fn with_evidence(conn: &rusqlite::Connection, err: String) -> String {
    ...
    format!("{err} [저장소 실황: {}]", soksak_store::integrity::failure_evidence(conn, &super::process_probe::process_memory_probe()))
}
...
fn write_with_retry<T>(
    ...
    soksak_store::integrity::with_nomem_retry(WRITE_ATTEMPTS, WRITE_BACKOFF, op)
...
pub fn data_repair(state: State<'_, DbState>) -> Result<soksak_store::integrity::Repair, String> {
    with_conn(&state, |c| soksak_store::integrity::repair(c))
}
