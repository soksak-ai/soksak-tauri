...
pub const UNSERVED: &[Unserved] = &[
    ...
    Unserved {
        ...
        blocked_by: "봉인 열쇠의 소유가 볼트에 있다. 이 프로세스가 쓰는 SealKeys 구현은 NoSealKeys 이고, 그것을 \
                     그대로 세우면 컴파일도 되고 답도 나온다 — 잠긴 볼트가 '열쇠 없음'과 같은 모양으로 나온다. \
                     조용한 오답이 오류보다 나쁘다. 게다가 새 열쇠를 만드는 쪽(enable·recover·rotate)은 그것을 \
                     볼트에 **보관**해야 하는데 SealKeys 계약에 그 자리가 없다(unlocked·new_key·secret 뿐). \
                     계약을 먼저 넓히지 않고 이름만 올리면 열쇠가 어디에도 안 남는다.",
    },
    Unserved {
    ...
    Unserved {
        name: "secret_set",
        blocked_by: "볼트 자체다. 셋이 함께 막는다. ① 백엔드가 OS 키체인이라 의존이 플랫폼 크레이트를 끌고 \
                     오고(그중 하나는 이 프로세스의 금지 이름이다) ② KEK 가 **앱 신원 ACL** 에 결속되어 다른 \
                     서명 신원의 프로세스는 같은 항목에 닿지 못한다 ③ 읽기가 쓰기다 — 상태 조회가 없는 볼트를 \
                     만드는 경로를 타므로, 관측이라 믿고 부르면 파일이 생긴다. 부팅이 키체인을 건드리지 않는 \
                     규칙과도 정면으로 부딪힌다.",
    },
    Unserved {
        name: "secret_has",
