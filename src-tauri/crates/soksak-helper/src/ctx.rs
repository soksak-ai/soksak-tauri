//! 헬퍼 프로세스의 부팅 상태 — **띄운 쪽이 준 값**이지 헬퍼가 추측한 값이 아니다.
//!
//! 처음 판은 이 상태를 아예 두지 않고 매 호출마다 정체성을 인자로 요구했다. 근거는
//! "헬퍼는 자기 정체성을 추측하지 않는다"였고 그 근거 자체는 옳다. 틀린 것은 결론이다:
//! **받는 것과 추측하는 것은 다르다.** 앱도 자기 정체성을 추측하지 않는다 — 부팅 때 셸
//! 설정에서 받아(`home::init(app.config().identifier)`) 전역에 세운다. 헬퍼는 띄운 쪽의
...
//! 그래서 헬퍼가 서빙한다고 믿은 5개가 전부 INVALID_PARAMS 로 거절됐고, 그 거절은 셸
...
/// 이 헬퍼가 서빙하는 대상. 하나의 헬퍼는 하나의 정체성을 서빙한다 — 여러 홈을 한
...
pub struct Ctx {
    ...
    /// 헬퍼가 규칙만 보고 파생하면 앱과 다른 파일을 열고, 그 오답은 오류가 아니라 빈 결과다.
    data_dir: PathBuf,
}
...
mod tests {
    ...
    /// 앱이 DB 를 옮겼으면 헬퍼도 그리로 가야 한다 — 규칙만 보고 파생하면 두 프로세스가
    ...
    /// 헬퍼의 홈 레이아웃은 앱의 것과 **같은 함수**에서 나온다.
    #[test]
